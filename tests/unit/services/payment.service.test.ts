  ...
  const instance = {
    ...
    plans: {
      create: jest.fn().mockResolvedValue({
        id: 'plan_test_1',
        item: { name: 'Hasivu monthly plan', amount: 999, currency: 'INR' },
      }),
    },
    subscriptions: {
      create: jest.fn().mockResolvedValue({
...
describe('PaymentService', () => {
  ...
  describe('createPaymentOrder', () => {
    ...
    beforeEach(() => {
      ...
      const p = getPrismaMock();
      p.paymentOrder.create.mockResolvedValue({
        id: 'po-1',
  ...
  describe('getPaymentOrder', () => {
    it('returns first payment order or null', async () => {
      getPrismaMock().paymentOrder.findFirst.mockResolvedValueOnce(null);
      expect(await paymentService.getPaymentOrder('ord')).toBeNull();

      getPrismaMock().paymentOrder.findFirst.mockResolvedValueOnce({
        id: 'po-1',
        razorpayOrderId: 'order_razorpay_123',
        status: 'created',
      });
      expect(await paymentService.getPaymentOrder('ord')).toMatchObject({ id: 'po-1' });
    });
  });

  describe('createSubscriptionPlan / createSubscription', () => {
    it('returns a plan-shaped object', async () => {
      mockRazorpay.plans = mockRazorpay.plans || { create: jest.fn() };
      (mockRazorpay as { plans: { create: jest.Mock } }).plans.create.mockResolvedValue({
        id: 'plan_test_1',
        item: { name: 'Hasivu monthly plan', amount: 999, currency: 'INR' },
      });
      ...
      expect(plan.id).toBe('plan_test_1');
    });

    it('returns a subscription-shaped object', async () => {
      mockRazorpay.subscriptions = mockRazorpay.subscriptions || { create: jest.fn() };
      (mockRazorpay as { subscriptions: { create: jest.Mock } }).subscriptions.create.mockResolvedValue({
        id: 'sub_test_1',
        status: 'created',
        plan_id: 'p1',
      });
  ...
  describe('static processPayment', () => {
    it('creates and completes a payment via getInstance', async () => {
      PaymentService.resetInstanceForTests();
      const p = getPrismaMock();
      p.order.findUnique.mockResolvedValue({ id: 'ord-1', userId: 'user-123' });
      p.payment.create.mockResolvedValue(stubPayment({ id: 'new1', status: 'pending' }));
      p.payment.findUnique.mockResolvedValue(stubPayment({ id: 'new1', status: 'pending' }));
      p.payment.update.mockResolvedValue(stubPayment({ id: 'new1', status: 'completed' }));
      jest.spyOn(PaymentService.getInstance(), 'processPayment').mockResolvedValue(
        stubPayment({ id: 'new1', status: 'completed', razorpayPaymentId: 'pay_test' }) as never
      );
