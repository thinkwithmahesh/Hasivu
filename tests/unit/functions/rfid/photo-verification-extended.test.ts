/**
 * HASIVU Platform - Extended Photo Verification Lambda Function Tests
 * Comprehensive test coverage for photo verification with Phase 2.1 enhancements
 * Tests photo upload, analysis, S3 storage, and delivery verification
...
describe('Extended Photo Verification - Phase 2.1', () => {
  ...
  beforeEach(() => {
    ...
    (mockPrisma.deliveryVerification.findUnique as jest.Mock).mockResolvedValue(mockDeliveryVerification);
    ...
    (secureRegex.test as jest.Mock).mockReturnValue({ isMatch: true, error: null });
  });
  ...
  describe('Phase 2.1 Enhanced Photo Upload and Verification', () => {
    ...
    it('should successfully process and store photo verification', async () => {
      ...
      const result = await photoVerificationHandler(event, mockContext);
    ...
    it('should validate photo data format and size constraints', async () => {
      const oversizedImage = `data:image/jpeg;base64,${  'x'.repeat(12 * 1024 * 1024)}`; // 12MB
    ...
    it('should perform AI-powered photo analysis', async () => {
      ...
      await photoVerificationHandler(event, mockContext);

      // Verify analysis was performed (mocked)
    ...
    it('should handle photo upload request with presigned URLs', async () => {
      ...
      const result = await photoUploadRequestHandler(event, mockContext);
    ...
    it('should retrieve photo verification details', async () => {
      (mockPrisma.deliveryVerification.findUnique as jest.Mock).mockResolvedValue({
      ...
      const result = await getPhotoVerificationHandler(event, mockContext);
  ...
  describe('Phase 2.1 Security and Access Control', () => {
    ...
    it('should enforce proper authorization for photo operations', async () => {
      ...
      expect(result.statusCode).toBe(500); // Authorization check fails
    });
    ...
    it('should prevent access to non-existent verifications', async () => {
      (mockPrisma.deliveryVerification.findUnique as jest.Mock).mockResolvedValue(null);
    ...
    it('should validate photo content types', async () => {
      const event: APIGatewayProxyEvent = {
        ...
        body: JSON.stringify({
          verificationId: 'verification-photo-789',
          contentType: 'image/gif', // Invalid type
          fileSize: 500000
        }),
        headers: {
          'authorization': 'Bearer valid-photo-token',
  ...
  describe('Phase 2.1 Error Handling and Recovery', () => {
    ...
    it('should handle invalid base64 photo data', async () => {
      (secureRegex.test as jest.Mock).mockReturnValue({ isMatch: false, error: 'Invalid format' });
  ...
  describe('Phase 2.1 Performance and Scalability', () => {
    ...
    it('should handle concurrent photo verification requests', async () => {
      ...
      const [result1, result2] = await Promise.all([
        photoVerificationHandler(event1, mockContext),
        photoVerificationHandler(event2, mockContext)
      ]);
    ...
    it('should handle large photo uploads efficiently', async () => {
      const largeValidImage = `data:image/jpeg;base64,${  'x'.repeat(5 * 1024 * 1024)}`; // 5MB
  ...
  describe('Phase 2.1 Data Integrity and Consistency', () => {
    ...
    it('should create comprehensive audit trails', async () => {
      ...
      // Verify audit logging (would be in the actual implementation)
      expect(mockPrisma.$transaction).toHaveBeenCalled();
    });

    it('should handle partial failures in multi-step operations', async () => {
      // Mock S3 success but database failure
      (mockPrisma.deliveryVerification.update as jest.Mock).mockRejectedValue(new Error('Database update failed'));
