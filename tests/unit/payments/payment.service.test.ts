 ...
 * Tests for the fixed payment service implementation
 */
...
describe('Payment Service - Comprehensive Tests', () => {
  ...
  beforeEach(() => {
    ...
    (DatabaseService as any).client = mockDatabaseService.client;
    (DatabaseService as any).transaction = mockDatabaseService.transaction;
    ...
    (RedisService as any).setex = mockRedisService.setex;
    (RedisService as any).get = mockRedisService.get;
    (RedisService as any).del = mockRedisService.del;
    ...
    (paymentService as any).webhookSecret = 'test-webhook-secret';
  });
  ...
  describe('Create Payment Order', () => {
    ...
    test('should create payment order successfully', async () => {
      ...
      expect(mockRedisService.setex).toHaveBeenCalledWith(
        ...
        900,
        JSON.stringify(mockDatabaseOrder)
      );
    });
  ...
  describe('Capture Payment', () => {
    ...
    test('should capture authorized payment successfully', async () => {
      ...
      const result = await paymentService.capturePayment(
        validCaptureData.orderId,
        validCaptureData.paymentId,
        validSignature
      );
    ...
    test('should handle already captured payment', async () => {
      ...
      // Should not call capture again for already captured payment
    ...
    test('should reject capture with invalid signature', async () => {
      ...
      await expect(paymentService.capturePayment(
        'order_razorpay_123',
        'pay_razorpay_123',
        'invalid_signature'
  ...
  describe('Security Tests', () => {
    test('should use timing-safe signature comparison', () => {
      ...
      // Test that the function uses crypto.timingSafeEqual for comparison
