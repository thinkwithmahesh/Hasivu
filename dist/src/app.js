...
class SimpleApp {
    ...
    async loadInputValidationMiddleware() {
        try {
            const { comprehensiveInputValidation } = await Promise.resolve().then(() => __importStar(require('./middleware/input-validation.middleware')));
            this.app.use(comprehensiveInputValidation);
            logger_1.logger.info('Comprehensive input validation middleware loaded successfully');
        }
        catch (err) {
            logger_1.logger.warn('Input validation middleware failed to load, continuing without comprehensive validation', err);
        }
    }
    async loadRateLimiterMiddleware() {
        try {
            const { generalRateLimit } = await Promise.resolve().then(() => __importStar(require('./middleware/rateLimiter.middleware')));
            this.app.use(generalRateLimit);
            logger_1.logger.info('Rate limiter middleware loaded successfully');
        }
        catch (err) {
            logger_1.logger.warn('Rate limiter middleware failed to load, continuing without rate limiting', err);
        }
    }
    async loadCSRFMiddleware() {
        try {
            const { csrfProtection, attachCSRFToken } = await Promise.resolve().then(() => __importStar(require('./middleware/csrf.middleware')));
            this.app.use(attachCSRFToken);
            this.app.use('/api', csrfProtection());
            logger_1.logger.info('CSRF middleware loaded successfully');
        }
        catch (err) {
            logger_1.logger.warn('CSRF middleware failed to load, continuing without CSRF protection', err);
        }
    }
    setupRoutes() {
