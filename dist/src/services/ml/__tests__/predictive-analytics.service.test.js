...
describe('PredictiveAnalyticsService', () => {
    let service;
    let mockMLflow;
    let mockModelArtifact;
    let mockRealtimePrediction;
    let mockFeatureEngineering;
    let mockRecommendationEngine;
    let mockModelMonitoring;
    let mockExplainability;
    beforeEach(async () => {
        jest.clearAllMocks();
        service = predictive_analytics_service_1.PredictiveAnalyticsService.getInstance();
        mockMLflow = service['mlflow'];
        mockModelArtifact = service['modelArtifact'];
        mockRealtimePrediction = service['realtimePrediction'];
        mockFeatureEngineering = service['featureEngineering'];
        mockRecommendationEngine = service['recommendationEngine'];
        mockModelMonitoring = service['modelMonitoring'];
        mockExplainability = service['explainability'];
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('Prediction', () => {
        ...
        it('should make successful prediction with all features', async () => {
            ...
            expect(mockFeatureEngineering.extractFeatures).toHaveBeenCalledWith(samplePredictionRequest.inputData, samplePredictionRequest.modelType, samplePredictionRequest.schoolId);
        ...
        it('should handle personalization parameters', async () => {
            ...
            expect(mockRealtimePrediction.predict).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('Model Training', () => {
        ...
        beforeEach(() => {
            const mockAutoML = service['autoML'];
        ...
        it('should train model successfully', async () => {
            const modelId = await service.trainModel('student_behavior', sampleTrainingData, sampleConfig);
        ...
        it('should register model with MLflow', async () => {
            ...
            expect(mockMLflow.registerModel).toHaveBeenCalledWith('model_12345', expect.objectContaining({
                modelType: 'student_behavior',
                config: sampleConfig,
        ...
        it('should add model to monitoring', async () => {
            await service.trainModel('student_behavior', sampleTrainingData, sampleConfig);
            expect(mockModelMonitoring.addModel).toHaveBeenCalledWith('model_12345', expect.objectContaining({
                modelType: 'student_behavior'
            }));
        });
        it('should handle federated learning training', async () => {
            ...
            const mockFederatedLearning = service['federatedLearning'];
            mockFederatedLearning.trainFederatedModel = jest.fn().mockResolvedValue('federated_model_123');
            const modelId = await service.trainModel('student_behavior', sampleTrainingData, sampleConfig, federatedConfig);
    ...
    describe('Analytics', () => {
        ...
        it('should filter analytics by school ID', async () => {
            ...
            expect(mockModelMonitoring.getPerformanceReport).toHaveBeenCalledWith(schoolId, undefined);
        });
        ...
        it('should filter analytics by time range', async () => {
            ...
            expect(mockModelMonitoring.getPerformanceReport).toHaveBeenCalledWith(undefined, timeRange);
        });
    });
    describe('Model Retraining', () => {
        beforeEach(() => {
            mockModelMonitoring.checkDrift = jest.fn().mockResolvedValue(0.15);
            const mockAutoML = service['autoML'];
        ...
        it('should skip retraining when drift is below threshold', async () => {
            mockModelMonitoring.checkDrift = jest.fn().mockResolvedValue(0.05);
            const result = await service.retrain('student_behavior', 'school-123');
    ...
    describe('Recommendations', () => {
        ...
        it('should get personalized recommendations', async () => {
            const result = await service.getRecommendations('user-123', 'student', 'school-123', { current_meal_time: 'lunch' });
    ...
    describe('Error Handling', () => {
        ...
        it('should handle timeout errors', async () => {
            ...
            mockRealtimePrediction.predict = jest.fn().mockImplementation(() => new Promise((resolve) => setTimeout(resolve, 2000)));
            const request = {
                modelType: 'student_behavior',
                schoolId: 'school-123',
                inputData: { test: 'data' }
...
//# sourceMappingURL=predictive-analytics.service.test.js.map