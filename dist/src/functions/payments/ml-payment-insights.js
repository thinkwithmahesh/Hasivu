...
exports.handler = exports.mlPaymentInsightsHandler = void 0;
...
const mlInsightsQuerySchema = zod_1.z.object({
    schoolId: zod_1.z.string().uuid().optional(),
    timeframe: zod_1.z.enum(['week', 'month', 'quarter', 'year']).default('month'),
    insightTypes: zod_1.z.array(zod_1.z.enum(['anomaly', 'churn', 'revenue_forecast', 'trend_analysis', 'all'])).default(['all']),
    includeRecommendations: zod_1.z.boolean().default(true),
    confidenceThreshold: zod_1.z.number().min(0).max(1).default(0.7)
});
const trainModelSchema = zod_1.z.object({
    schoolId: zod_1.z.string().uuid().optional(),
    modelType: zod_1.z.enum(['churn_prediction', 'revenue_forecast', 'anomaly_detection', 'payment_success_prediction']),
    trainingPeriodMonths: zod_1.z.number().min(3).max(24).default(12),
    features: zod_1.z.array(zod_1.z.string()).optional(),
    hyperparameters: zod_1.z.object({
        learningRate: zod_1.z.number().min(0.001).max(1).default(0.01),
        maxDepth: zod_1.z.number().min(1).max(20).default(6),
        minSamplesLeaf: zod_1.z.number().min(1).max(100).default(5),
        nEstimators: zod_1.z.number().min(10).max(1000).default(100)
    }).optional()
});
async function detectPaymentAnomalies(startDate, endDate, schoolId, confidenceThreshold = 0.7) {
    ...
    payments.forEach(payment => {
        ...
        if (deviation > 3 && deviation > confidenceThreshold * 4) {
            anomalies.push({
    ...
    const failureRateStd = Math.sqrt(dailyFailureRates.reduce((sum, rate) => sum + Math.pow(rate - failureRateMean, 2), 0) / dailyFailureRates.length);
...
function calculateDailyFailureRates(payments) {
    const dailyStats = {};
    payments.forEach(payment => {
    ...
    return Object.values(dailyStats).map(stat => stat.total > 0 ? stat.failed / stat.total : 0);
}
async function predictChurnRisk(startDate, endDate, schoolId) {
    const prismaClient = getPrismaClient();
    const predictions = [];
    const whereClause = {};
    if (schoolId) {
    ...
    for (const school of schools) {
        ...
        const activeSubscriptions = school.users.flatMap(user => user.subscriptions.filter(sub => sub.status === 'active'));
        ...
        const paymentFrequency = allPayments.length / Math.max(1, Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24 * 30)));
        ...
        let churnScore = 0;
        const factors = [];
        const daysFactor = Math.min(1, daysSinceLastPayment / 90);
        ...
        const frequencyFactor = Math.max(0, 1 - paymentFrequency / 2);
        ...
        const revenueFactor = Math.max(0, 1 - averageOrderValue / 1000);
        ...
        predictions.push({
            ...
            revenueAtRisk,
            confidenceScore: 0.75 + (allPayments.length / 100) * 0.2,
            lastPaymentDate,
...
async function generateRevenueForecast(startDate, endDate, schoolId) {
    const prismaClient = getPrismaClient();
    const forecasts = [];
    const whereClause = {
        createdAt: {
...
function performTimeSeriesForecasting(data, forecastType) {
    if (data.length < 3)
        return null;
...
function calculateSeasonality(revenues) {
...
function getNextPeriod(lastPeriod, type) {
...
async function generateTrendAnalysis(startDate, endDate, schoolId) {
    const prismaClient = getPrismaClient();
    const trends = [];
    const whereClause = {
        createdAt: {
...
function analyzeTrend(payments, metricType) {
    if (payments.length < 7)
        return null;
    let values = [];
    let drivingFactors = [];
...
function calculateDailySuccessRates(payments) {
    const dailyStats = {};
    payments.forEach(payment => {
        const day = payment.createdAt.toISOString().split('T')[0];
        if (!dailyStats[day]) {
    ...
    return Object.values(dailyStats).map(stat => stat.total > 0 ? stat.successful / stat.total : 0);
}
function calculateDailyAverageAmounts(payments) {
    const dailyAmounts = {};
    payments.forEach(payment => {
        const day = payment.createdAt.toISOString().split('T')[0];
        if (!dailyAmounts[day]) {
    ...
    return Object.values(dailyAmounts).map(amounts => amounts.length > 0 ? amounts.reduce((sum, amt) => sum + amt, 0) / amounts.length : 0);
}
function calculateDailyTransactionCounts(payments) {
    const dailyCounts = {};
    payments.forEach(payment => {
        const day = payment.createdAt.toISOString().split('T')[0];
...
function calculateTrendMetrics(values) {
    if (values.length < 3) {
...
function predictValue(values, daysAhead) {
    if (values.length < 2)
...
async function generateMLInsightsSummary(startDate, endDate, schoolId, insightTypes = ['all'], confidenceThreshold = 0.7) {
    ...
    if (healthScore >= 80)
        ...
        riskLevel = 'critical';
    const keyRecommendations = generateKeyRecommendations(anomalies, churnPredictions, trendAnalysis);
...
function generateKeyRecommendations(anomalies, churnPredictions, trends) {
    ...
    const negativeStrongTrends = trends.filter(t => t.direction === 'decreasing' && (t.significance === 'strong' || t.significance === 'very_strong'));
    ...
    const positiveStrongTrends = trends.filter(t => t.direction === 'increasing' && (t.significance === 'strong' || t.significance === 'very_strong'));
...
function assessDataQuality(startDate, endDate, totalDataPoints) {
    const daysDiff = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
    const expectedMinPoints = daysDiff * 0.7;
    ...
    const recency = Math.max(0, 100 - daysSinceEnd * 5);
...
function extractModelFeatures(data, requestedFeatures) {
...
const mlPaymentInsightsHandler = async (event, context) => {
...
exports.mlPaymentInsightsHandler = mlPaymentInsightsHandler;
async function handleMLInsightsQuery(queryParams, authenticatedUser, requestId) {
    const logger = logger_service_1.LoggerService.getInstance();
    ...
    try {
        const insights = await generateMLInsightsSummary(startDate, endDate, schoolId, insightsQuery.insightTypes, insightsQuery.confidenceThreshold);
...
async function handleModelTraining(event, authenticatedUser, requestId) {
    const logger = logger_service_1.LoggerService.getInstance();
    if (authenticatedUser.role !== 'super_admin') {
        return (0, response_utils_1.createErrorResponse)('Model training requires super_admin permissions', 403, 'INSUFFICIENT_PERMISSIONS');
    }
...
exports.handler = exports.mlPaymentInsightsHandler;
//# sourceMappingURL=ml-payment-insights.js.map