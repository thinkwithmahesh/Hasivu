...
const analytics_orchestrator_1 = require("../analytics-orchestrator");
...
describe('Analytics Orchestrator', () => {
    ...
    describe('Handler Authentication', () => {
        it('should reject unauthenticated requests', async () => {
            ...
            const result = await (0, analytics_orchestrator_1.analyticsOrchestratorHandler)(mockEvent, mockContext);
    ...
    describe('Operation Execution', () => {
        ...
        it('should handle invalid request bodies', async () => {
            ...
            const result = await (0, analytics_orchestrator_1.analyticsOrchestratorHandler)(invalidEvent, mockContext);
    ...
    describe('System Health Monitoring', () => {
        it('should return system health metrics', async () => {
            ...
            const { authenticateLambda } = require('../../shared/middleware/lambda-auth.middleware');
            authenticateLambda.mockResolvedValue(mockAuthResult);
            const result = await (0, analytics_orchestrator_1.analyticsOrchestratorHandler)(healthEvent, mockContext);
            expect(result.statusCode).toBe(200);
            const body = JSON.parse(result.body);
    ...
    describe('Cache Statistics', () => {
        it('should return cache statistics', async () => {
            ...
            const { authenticateLambda } = require('../../shared/middleware/lambda-auth.middleware');
            authenticateLambda.mockResolvedValue(mockAuthResult);
            const result = await (0, analytics_orchestrator_1.analyticsOrchestratorHandler)(cacheEvent, mockContext);
            expect(result.statusCode).toBe(200);
            const body = JSON.parse(result.body);
    ...
    describe('Operation Status Tracking', () => {
        it('should return operation status', async () => {
            ...
            const { authenticateLambda } = require('../../shared/middleware/lambda-auth.middleware');
            authenticateLambda.mockResolvedValue(mockAuthResult);
            const result = await (0, analytics_orchestrator_1.analyticsOrchestratorHandler)(statusEvent, mockContext);
            expect(result.statusCode).toBe(200);
        });
    ...
    describe('Batch Operations', () => {
        it('should handle batch operation requests', async () => {
            ...
            const { authenticateLambda } = require('../../shared/middleware/lambda-auth.middleware');
            authenticateLambda.mockResolvedValue(mockAuthResult);
            const result = await (0, analytics_orchestrator_1.analyticsOrchestratorHandler)(batchEvent, mockContext);
            expect(result.statusCode).toBe(200);
            const body = JSON.parse(result.body);
    ...
    describe('Error Handling', () => {
        ...
        it('should handle unsupported HTTP methods', async () => {
            ...
            const { authenticateLambda } = require('../../shared/middleware/lambda-auth.middleware');
            authenticateLambda.mockResolvedValue(mockAuthResult);
            const result = await (0, analytics_orchestrator_1.analyticsOrchestratorHandler)(invalidMethodEvent, mockContext);
    ...
    describe('AnalyticsOrchestrator Class', () => {
        let orchestrator;
        beforeEach(() => {
            orchestrator = new analytics_orchestrator_1.AnalyticsOrchestrator();
        });
        ...
        it('should queue operations correctly', async () => {
            const operationId = await orchestrator.queueOperation('test_operation', { param1: 'value1' }, 'user123', 'school123');
...
//# sourceMappingURL=analytics-orchestrator.test.js.map