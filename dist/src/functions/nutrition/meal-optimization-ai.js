...
const mealOptimizationRequestSchema = zod_1.z.object({
    ...
    constraints: zod_1.z.object({
        ...
        timeConstraints: zod_1.z.object({
    ...
    targets: zod_1.z.object({
        ...
        nutritionalBalance: zod_1.z.object({
            ...
            fatRatio: zod_1.z.number().min(0).max(1).default(0.30)
        })
    }),
...
async function getNutritionalData(foodIds) {
    ...
    try {
        ...
        logger_service_1.LoggerService.getInstance().error('Failed to fetch nutritional data', { error, foodIds });
...
Focus on practical, achievable improvements that align with the user's constraints and preferences.`;
        ...
        logger_service_1.LoggerService.getInstance().error('AI meal optimization failed', { error });
...
const mealOptimizationHandler = async (event, context) => {
    ...
    try {
        ...
        if (error.name === 'ZodError') {
            return (0, response_utils_1.createErrorResponse)('Invalid optimization request data', 400, 'VALIDATION_ERROR');
        }
