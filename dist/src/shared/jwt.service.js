...
class JWTService {
    ...
    validateJWTConfiguration() {
        ...
        if (issues.length > 0) {
            logger_service_1.logger.error('JWT configuration validation failed', undefined, {
                issues,
            });
            throw new Error(`JWT configuration issues: ${issues.join(', ')}`);
        }
        logger_service_1.logger.info('JWT configuration validated successfully', {
            issuer: this.issuer,
        ...
        try {
            ...
            if (authHeader) {
                ...
                if (bearerMatch) {
                    ...
                    if (token.length > 0) {
                        logger_service_1.logger.debug('JWT token found in Authorization header');
                        return token;
                    }
                }
            }
            ...
            if (authContext?.accessToken) {
                logger_service_1.logger.debug('JWT token found in authorizer context');
                return authContext.accessToken;
            }
            const queryToken = event.queryStringParameters?.token;
            if (queryToken && queryToken.length > 0) {
                logger_service_1.logger.debug('JWT token found in query parameter');
                return queryToken;
            }
            ...
            if (cookieHeader) {
                ...
                if (cookieToken && cookieToken.length > 0) {
                    logger_service_1.logger.debug('JWT token found in cookies');
                    return cookieToken;
                }
            }
            const customHeaderName = environment_1.config.jwt.customHeaderName || 'x-access-token';
            const customToken = event.headers?.[customHeaderName] || event.headers?.[customHeaderName.toLowerCase()];
            if (customToken && customToken.length > 0) {
                logger_service_1.logger.debug('JWT token found in custom header');
                return customToken;
            }
            logger_service_1.logger.warn('No JWT token found in request', {
            ...
            logger_service_1.logger.error('Error extracting JWT token from event', undefined, {
    ...
    parseCookies(cookieHeader) {
        ...
        try {
            ...
            logger_service_1.logger.warn('Failed to parse cookies', {
                cookieHeader,
    ...
    verifyToken(token) {
        ...
        try {
            ...
            logger_service_1.logger.debug('JWT token verified successfully', {
            ...
            logger_service_1.logger.warn('JWT token verification failed', {
                error: errorMessage,
                errorCode,
    ...
    generateAccessToken(payload, options = {}) {
        try {
            ...
            logger_service_1.logger.info('Access token generated successfully', {
            ...
            logger_service_1.logger.error(errorMessage, undefined, {
                userId: payload.userId,
                email: payload.email,
    ...
    generateRefreshToken(payload, options = {}) {
        try {
            ...
            logger_service_1.logger.info('Refresh token generated successfully', {
                userId: payload.userId,
                email: payload.email,
    ...
    verifyRefreshToken(token) {
        ...
        try {
            ...
            const expiresAt = decoded.exp ? new Date(decoded.exp * 1000) : undefined;
            const remainingTTL = decoded.exp ? Math.max(0, decoded.exp - now) : undefined;
            logger_service_1.logger.debug('Refresh token verified successfully', {
                userId: decoded.userId,
                email: decoded.email,
                role: decoded.role,
            ...
            logger_service_1.logger.warn('Refresh token verification failed', {
                error: errorMessage,
                errorCode,
                tokenLength: token.length,
            });
    ...
    async refreshAccessToken(refreshToken) {
        try {
            ...
            logger_service_1.logger.info('Access token refreshed successfully', {
                userId: refreshResult.payload.userId,
                email: refreshResult.payload.email,
                sessionId: refreshResult.payload.sessionId,
    ...
    decodeToken(token) {
        try {
            ...
            logger_service_1.logger.warn('Failed to decode JWT token', {
                error: error instanceof Error ? error.message : 'Unknown error',
                tokenLength: token.length,
            });
            return null;
    ...
    generateTokenPair(payload, options = {}) {
        try {
            ...
            const expiresIn = decoded.exp ? decoded.exp - Math.floor(Date.now() / 1000) : 900;
            logger_service_1.logger.info('JWT token pair generated successfully', {
                userId: payload.userId,
                email: payload.email,
                role: payload.role,
                sessionId: payload.sessionId,
    ...
    async revokeToken(token) {
        try {
            ...
            if (!decoded || !decoded.jti) {
                logger_service_1.logger.warn('Cannot revoke token without JTI', { tokenLength: token.length });
                return false;
            }
            logger_service_1.logger.info('Token revoked', {
                jti: decoded.jti,
                userId: decoded.userId,
                email: decoded.email,
            ...
            logger_service_1.logger.error('Failed to revoke token', undefined, {
                errorMessage: error instanceof Error ? error.message : 'Unknown error',
            });
            return false;
        }
