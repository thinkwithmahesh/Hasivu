...
const authenticateJWT = async (event, options = {}) => {
    ...
    try {
        ...
        if (!token) {
            logger_service_1.logger.warn('No JWT token provided in request', {
                path: event.path,
        ...
        if (options.requiredRole && !jwt_service_1.jwtService.hasRole(payload, options.requiredRole)) {
            logger_service_1.logger.warn('Insufficient role privileges', {
        ...
        if (options.requireBusinessContext && !payload.businessId) {
            logger_service_1.logger.warn('Business context required but not provided', {
                userId: payload.userId,
                path: event.path,
                method: event.httpMethod,
            });
        ...
        if (options.requireSessionValidation && !payload.sessionId) {
            logger_service_1.logger.warn('Session validation required but session ID not found', {
                userId: payload.userId,
                path: event.path,
                method: event.httpMethod,
            });
        ...
        if (options.customValidation) {
            try {
                ...
                if (!isValid) {
                    logger_service_1.logger.warn('Custom validation failed', {
                        userId: payload.userId,
                        path: event.path,
                        method: event.httpMethod,
                    });
                ...
                logger_service_1.logger.error('Custom validation error', error instanceof Error ? error : new Error(String(error)), {
                    userId: payload.userId,
                    path: event.path,
                    method: event.httpMethod,
                });
        ...
        logger_service_1.logger.info('Authentication successful', {
            userId: payload.userId,
            email: payload.email,
            role: payload.role,
        ...
        const duration = Date.now() - startTime;
        logger_service_1.logger.error('Authentication middleware error', error instanceof Error ? error : new Error(String(error)), {
            path: event.path,
            method: event.httpMethod,
            sourceIp: event.requestContext.identity.sourceIp,
            duration,
...
const rateLimitMiddleware = async (event, limits = {
    ...
    try {
        ...
        logger_service_1.logger.info('Rate limit check', {
            ...context,
            limits,
        ...
        logger_service_1.logger.error('Rate limiting error', error instanceof Error ? error : new Error(String(error)), {
            path: event.path,
            method: event.httpMethod,
        });
        return {
...
const withAuth = (handler, authOptions = {}) => {
    return async (event) => {
        try {
            ...
            logger_service_1.logger.error('Authentication wrapper error', error instanceof Error ? error : new Error(String(error)), {
                path: event.path,
                method: event.httpMethod,
            });
...
const withOptionalAuth = (handler) => {
    return async (event) => {
        try {
            ...
            logger_service_1.logger.error('Optional authentication wrapper error', error instanceof Error ? error : new Error(String(error)), {
                path: event.path,
                method: event.httpMethod,
            });
            return (0, exports.createAuthErrorResponse)(500, 'Internal server error', 'INTERNAL_ERROR');
