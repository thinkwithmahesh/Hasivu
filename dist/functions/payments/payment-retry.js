...
const retryPaymentSchema = zod_1.z.object({
    ...
    delayMinutes: zod_1.z.number().int().min(1).max(1440).optional(), // Max 24 hours
...
function calculateSmartDelay(attemptCount, previousFailures) {
    ...
    const calculatedDelay = Math.min(baseDelay * exponentialFactor * patternMultiplier * (1 + jitter), 120 // Maximum 2 hours
    );
