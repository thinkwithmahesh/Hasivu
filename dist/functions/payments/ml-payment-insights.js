...
async function predictChurnRisk(startDate, endDate, schoolId) {
    ...
    for (const school of schools) {
        ...
        const paymentFrequency = allPayments.length / Math.max(1, Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24 * 30))); // payments per month
...
async function generateTrendAnalysis(startDate, endDate, schoolId) {
    ...
    if (payments.length < 7)
        return trends; // Need at least a week of data
