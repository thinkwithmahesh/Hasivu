...
describe('Extended Mobile RFID Card Management - Phase 2.1', () => {
    ...
    beforeEach(() => {
        ...
        mockPrisma.deliveryVerification.count.mockResolvedValue(5);
        mockPrisma.deliveryVerification.findFirst
            .mockResolvedValueOnce({ createdAt: new Date('2024-01-16') })
            .mockResolvedValueOnce({ createdAt: new Date('2024-01-10') });
        mockPrisma.deliveryVerification.findMany.mockResolvedValue([
    ...
    describe('Phase 2.1 Enhanced Card Status Retrieval', () => {
        ...
        it('should return comprehensive usage statistics', async () => {
            ...
            await (0, mobile_card_management_1.getRfidCardStatus)(event);
    ...
    describe('Phase 2.1 Enhanced Issue Reporting', () => {
        it('should successfully report RFID card issues with validation', async () => {
            ...
            expect(createErrorResponse).toHaveBeenCalledWith('VALIDATION_ERROR', 'Invalid request data: studentId is required', 400);
        });
        it('should validate issue report data thoroughly', async () => {
            const invalidEvent = {
                ...
                body: JSON.stringify({
                    ...
                    requestReplacement: 'not_boolean'
                }),
                requestContext: {},
                path: '/rfid/issues',
                isBase64Encoded: false,
            ...
            const result = await (0, mobile_card_management_1.reportRfidIssue)(invalidEvent);
            expect(result.statusCode).toBe(400);
            expect(createErrorResponse).toHaveBeenCalledWith('VALIDATION_ERROR', expect.stringContaining('Invalid request data'), 400);
        });
        it('should validate issue types are within allowed values', async () => {
            ...
            const result = await (0, mobile_card_management_1.reportRfidIssue)(event);
            expect(result.statusCode).toBe(400);
            expect(createErrorResponse).toHaveBeenCalledWith('VALIDATION_ERROR', expect.stringContaining('Invalid issue type'), 400);
        });
    ...
    describe('Phase 2.1 Security and Access Control', () => {
        ...
        it('should handle malformed JSON input securely', async () => {
            ...
            const result = await (0, mobile_card_management_1.reportRfidIssue)(malformedEvent);
            expect(result.statusCode).toBe(400);
            expect(createErrorResponse).toHaveBeenCalledWith('VALIDATION_ERROR', expect.stringContaining('Invalid request data'), 400);
        });
    });
    ...
    describe('Phase 2.1 Performance and Monitoring', () => {
        ...
        it('should handle concurrent mobile requests efficiently', async () => {
            ...
            const [result1, result2] = await Promise.all([
                (0, mobile_card_management_1.getRfidCardStatus)(event1),
                (0, mobile_card_management_1.getRfidCardStatus)(event2)
            ]);
...
//# sourceMappingURL=mobile-card-management-extended.test.js.map