...
describe('MenuItemService', () => {
    ...
    describe('createMenuItem', () => {
        ...
        it('should create menu item successfully', async () => {
            ...
            MockedMenuItemRepository.create.mockResolvedValue(mockMenuItem);
            const result = await menuItem_service_1.MenuItemService.createMenuItem(validInput);
            expect(result).toEqual(mockMenuItem);
            expect(MockedMenuItemRepository.findByNameAndSchool).toHaveBeenCalledWith('Pizza Margherita', 'school-123');
        ...
        it('should throw error for duplicate menu item name in same school', async () => {
            ...
            MockedMenuItemRepository.findByNameAndSchool.mockResolvedValue(existingItem);
            await expect(menuItem_service_1.MenuItemService.createMenuItem(validInput))
        ...
        it('should validate required fields', async () => {
            ...
            await expect(menuItem_service_1.MenuItemService.createMenuItem(invalidInput))
    ...
    describe('getMenuItemById', () => {
        it('should return cached menu item', async () => {
            ...
            const result = await menuItem_service_1.MenuItemService.getMenuItemById('item-123');
            expect(result).toEqual(mockMenuItem);
        ...
        it('should fetch and cache menu item when not cached', async () => {
            ...
            expect(MockedCache.setex).toHaveBeenCalledWith('menu_item:item-123:true', 300, JSON.stringify(mockMenuItem));
        });
        it('should return null for non-existent menu item', async () => {
            MockedCache.get.mockResolvedValue(null);
            MockedMenuItemRepository.findById.mockResolvedValue(null);
            const result = await menuItem_service_1.MenuItemService.getMenuItemById('nonexistent');
            expect(result).toBeNull();
            expect(logger_1.logger.warn).toHaveBeenCalledWith('Menu item not found', { menuItemId: 'nonexistent' });
        });
    });
    describe('getMenuItems', () => {
        it('should return paginated menu items with default parameters', async () => {
            const mockItems = [
                { id: 'item-1', name: 'Pizza', category: menuItem_service_1.MenuCategory.LUNCH },
                { id: 'item-2', name: 'Salad', category: menuItem_service_1.MenuCategory.SNACKS }
            ];
            const mockResult = { items: mockItems, total: 2 };
            MockedMenuItemRepository.findMany.mockResolvedValue(mockResult);
            const result = await menuItem_service_1.MenuItemService.getMenuItems();
        ...
        it('should apply filters correctly', async () => {
            ...
            MockedMenuItemRepository.findMany.mockResolvedValue(mockResult);
            await menuItem_service_1.MenuItemService.getMenuItems(filters);
            expect(MockedMenuItemRepository.findMany).toHaveBeenCalledWith({
                filters,
                skip: 0,
                take: 20,
        ...
        it('should handle pagination correctly', async () => {
            ...
            MockedMenuItemRepository.findMany.mockResolvedValue(mockResult);
            const result = await menuItem_service_1.MenuItemService.getMenuItems({}, pagination);
        ...
        it('should limit maximum items per page', async () => {
            ...
            const mockResult = { items: [], total: 0 };
            MockedMenuItemRepository.findMany.mockResolvedValue(mockResult);
            await menuItem_service_1.MenuItemService.getMenuItems({}, pagination);
            expect(MockedMenuItemRepository.findMany).toHaveBeenCalledWith(expect.objectContaining({ take: 100 }));
        });
    });
    describe('searchMenuItems', () => {
        it('should search menu items with term', async () => {
            const mockResult = { items: [{ id: 'item-1', name: 'Pizza' }], total: 1 };
            MockedMenuItemRepository.search.mockResolvedValue(mockResult);
            const result = await menuItem_service_1.MenuItemService.searchMenuItems('pizza');
            expect(result).toEqual(mockResult);
            expect(MockedMenuItemRepository.search).toHaveBeenCalledWith('pizza', {}, {});
        });
        it('should return all items for empty search term', async () => {
            const mockResult = { items: [], total: 0, page: 1, limit: 20, totalPages: 0 };
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItems').mockResolvedValue(mockResult);
            const result = await menuItem_service_1.MenuItemService.searchMenuItems('');
            expect(result).toEqual({ items: [], total: 0 });
            expect(menuItem_service_1.MenuItemService.getMenuItems).toHaveBeenCalledWith({}, {});
        });
        it('should apply filters during search', async () => {
            const filters = { category: menuItem_service_1.MenuCategory.DINNER };
            const mockResult = { items: [], total: 0 };
            MockedMenuItemRepository.search.mockResolvedValue(mockResult);
            await menuItem_service_1.MenuItemService.searchMenuItems('chocolate', filters);
            expect(MockedMenuItemRepository.search).toHaveBeenCalledWith('chocolate', filters, {});
        });
    });
    describe('updateMenuItem', () => {
        ...
        it('should update menu item successfully', async () => {
            ...
            MockedMenuItemRepository.findById.mockResolvedValue(existingItem);
            MockedMenuItemRepository.findByNameAndSchool.mockResolvedValue(null);
            MockedMenuItemRepository.update.mockResolvedValue(updatedItem);
            const result = await menuItem_service_1.MenuItemService.updateMenuItem('item-123', updateInput);
            expect(result).toEqual(updatedItem);
            expect(MockedMenuItemRepository.update).toHaveBeenCalled();
        });
        it('should throw error for non-existent menu item', async () => {
            MockedMenuItemRepository.findById.mockResolvedValue(null);
            await expect(menuItem_service_1.MenuItemService.updateMenuItem('nonexistent', updateInput))
        ...
        it('should check for duplicate names when updating', async () => {
            ...
            MockedMenuItemRepository.findById.mockResolvedValue(existingItem);
            MockedMenuItemRepository.findByNameAndSchool.mockResolvedValue(duplicateItem);
            await expect(menuItem_service_1.MenuItemService.updateMenuItem('item-123', updateInput))
        ...
        it('should validate price during update', async () => {
            ...
            await expect(menuItem_service_1.MenuItemService.updateMenuItem('item-123', invalidUpdate))
                .rejects.toThrow('Menu item price must be greater than 0');
        });
    });
    describe('deleteMenuItem', () => {
        it('should soft delete menu item by default', async () => {
            ...
            MockedMenuItemRepository.findById.mockResolvedValue(existingItem);
            MockedMenuItemRepository.update.mockResolvedValue(deletedItem);
            const result = await menuItem_service_1.MenuItemService.deleteMenuItem('item-123');
            expect(result).toEqual(deletedItem);
            expect(MockedMenuItemRepository.update).toHaveBeenCalledWith('item-123', { available: false });
        });
        it('should hard delete when specified', async () => {
            const existingItem = { id: 'item-123', name: 'Pizza' };
            MockedMenuItemRepository.findById.mockResolvedValue(existingItem);
            MockedMenuItemRepository.delete.mockResolvedValue(existingItem);
            const result = await menuItem_service_1.MenuItemService.deleteMenuItem('item-123', true);
            expect(result).toEqual(existingItem);
            expect(MockedMenuItemRepository.delete).toHaveBeenCalledWith('item-123');
        });
        it('should throw error for non-existent item', async () => {
            MockedMenuItemRepository.findById.mockResolvedValue(null);
            await expect(menuItem_service_1.MenuItemService.deleteMenuItem('nonexistent'))
                .rejects.toThrow('Menu item with ID nonexistent not found');
        });
    });
    describe('updateSortOrders', () => {
        it('should update sort orders for multiple items', async () => {
            ...
            const existingItems = [
                { id: 'item-1', schoolId: 'school-123', category: menuItem_service_1.MenuCategory.LUNCH },
                { id: 'item-2', schoolId: 'school-123', category: menuItem_service_1.MenuCategory.LUNCH }
            ];
            MockedMenuItemRepository.findById
                .mockResolvedValueOnce(existingItems[0])
                .mockResolvedValueOnce(existingItems[1]);
            MockedMenuItemRepository.batchUpdateSortOrders.mockResolvedValue(undefined);
            await menuItem_service_1.MenuItemService.updateSortOrders(updates);
        ...
        it('should throw error for missing items', async () => {
            ...
            MockedMenuItemRepository.findById.mockResolvedValue(null);
            await expect(menuItem_service_1.MenuItemService.updateSortOrders(updates))
        ...
        it('should handle empty updates array', async () => {
            await menuItem_service_1.MenuItemService.updateSortOrders([]);
    ...
    describe('toggleFeatured', () => {
        it('should toggle featured status from false to true', async () => {
            ...
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItemById').mockResolvedValue(mockItem);
            jest.spyOn(menuItem_service_1.MenuItemService, 'updateMenuItem').mockResolvedValue(updatedItem);
            const result = await menuItem_service_1.MenuItemService.toggleFeatured('item-123');
            expect(result).toEqual(updatedItem);
            expect(menuItem_service_1.MenuItemService.updateMenuItem).toHaveBeenCalledWith('item-123', { featured: true });
        });
        it('should throw error for non-existent item', async () => {
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItemById').mockResolvedValue(null);
            await expect(menuItem_service_1.MenuItemService.toggleFeatured('nonexistent'))
                .rejects.toThrow('Menu item with ID nonexistent not found');
        });
    });
    describe('getMenuItemsByAllergens', () => {
        it('should filter out items with excluded allergens', async () => {
            ...
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItems').mockResolvedValue(mockResult);
            const result = await menuItem_service_1.MenuItemService.getMenuItemsByAllergens(['nuts']);
    ...
    describe('getNutritionalSummary', () => {
        it('should calculate nutritional summary for multiple items', async () => {
            ...
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItemById')
                .mockResolvedValueOnce(mockItems[0])
                .mockResolvedValueOnce(mockItems[1]);
            const result = await menuItem_service_1.MenuItemService.getNutritionalSummary(['item-1', 'item-2']);
            expect(result).toEqual({
                totalCalories: 500,
        ...
        it('should handle missing items gracefully', async () => {
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItemById')
                .mockResolvedValueOnce({ id: 'item-1', calories: 300 })
                .mockResolvedValueOnce(null);
            const result = await menuItem_service_1.MenuItemService.getNutritionalSummary(['item-1', 'nonexistent']);
        ...
        it('should handle invalid nutritional JSON', async () => {
            ...
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItemById').mockResolvedValue(mockItem);
            const result = await menuItem_service_1.MenuItemService.getNutritionalSummary(['item-1']);
            expect(result).toEqual({
                totalCalories: 300,
                allergens: [],
                nutritionalInfo: {}
    ...
    describe('getFeaturedItems', () => {
        it('should get featured items with default options', async () => {
            ...
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItems').mockResolvedValue(mockResult);
            const result = await menuItem_service_1.MenuItemService.getFeaturedItems();
            expect(menuItem_service_1.MenuItemService.getMenuItems).toHaveBeenCalledWith({ featured: true, available: true }, { limit: 10, sortBy: 'sortOrder', sortOrder: 'asc' });
            expect(result).toEqual(mockResult.items);
        });
        it('should apply school and category filters', async () => {
            const options = { schoolId: 'school-123', category: menuItem_service_1.MenuCategory.DINNER, limit: 5 };
            const mockResult = { items: [], total: 0, page: 1, limit: 5, totalPages: 0 };
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItems').mockResolvedValue(mockResult);
            await menuItem_service_1.MenuItemService.getFeaturedItems(options);
            expect(menuItem_service_1.MenuItemService.getMenuItems).toHaveBeenCalledWith({
                featured: true,
                available: true,
                schoolId: 'school-123',
    ...
    describe('getMenuItemsByCategory', () => {
        it('should get items by category with default options', async () => {
            ...
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItems').mockResolvedValue(mockResult);
            const result = await menuItem_service_1.MenuItemService.getMenuItemsByCategory(menuItem_service_1.MenuCategory.LUNCH);
            expect(menuItem_service_1.MenuItemService.getMenuItems).toHaveBeenCalledWith({ category: menuItem_service_1.MenuCategory.LUNCH }, { limit: 50, sortBy: 'sortOrder', sortOrder: 'asc' });
            expect(result).toEqual(mockResult.items);
        });
        it('should apply additional filters', async () => {
            ...
            const mockResult = { items: [], total: 0, page: 1, limit: 20, totalPages: 0 };
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItems').mockResolvedValue(mockResult);
            await menuItem_service_1.MenuItemService.getMenuItemsByCategory(menuItem_service_1.MenuCategory.SNACKS, options);
            expect(menuItem_service_1.MenuItemService.getMenuItems).toHaveBeenCalledWith({
                category: menuItem_service_1.MenuCategory.SNACKS,
                schoolId: 'school-123',
                available: true
    ...
    describe('toggleAvailability', () => {
        it('should toggle availability from true to false', async () => {
            ...
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItemById').mockResolvedValue(mockItem);
            jest.spyOn(menuItem_service_1.MenuItemService, 'updateMenuItem').mockResolvedValue(updatedItem);
            const result = await menuItem_service_1.MenuItemService.toggleAvailability('item-123');
            expect(result).toEqual(updatedItem);
            expect(menuItem_service_1.MenuItemService.updateMenuItem).toHaveBeenCalledWith('item-123', { available: false });
        });
        it('should throw error for non-existent item', async () => {
            jest.spyOn(menuItem_service_1.MenuItemService, 'getMenuItemById').mockResolvedValue(null);
            await expect(menuItem_service_1.MenuItemService.toggleAvailability('nonexistent'))
                .rejects.toThrow('Menu item with ID nonexistent not found');
        });
    });
    describe('error handling', () => {
        it('should handle repository errors gracefully', async () => {
            ...
            await expect(menuItem_service_1.MenuItemService.getMenuItemById('item-123'))
                .rejects.toThrow('Database connection failed');
            expect(logger_1.logger.error).toHaveBeenCalledWith('Failed to get menu item by ID', error, { menuItemId: 'item-123' });
        });
        it('should handle cache errors gracefully', async () => {
            const cacheError = new Error('Redis connection failed');
            MockedCache.get.mockRejectedValue(cacheError);
            MockedMenuItemRepository.findById.mockResolvedValue({ id: 'item-123' });
            const result = await menuItem_service_1.MenuItemService.getMenuItemById('item-123');
    ...
    describe('cache management', () => {
        it('should clear relevant caches after create', async () => {
            ...
            await menuItem_service_1.MenuItemService.createMenuItem(validInput);
        ...
        it('should clear caches after update', async () => {
            ...
            MockedMenuItemRepository.findById.mockResolvedValue(existingItem);
            MockedMenuItemRepository.update.mockResolvedValue(existingItem);
            await menuItem_service_1.MenuItemService.updateMenuItem('item-123', { name: 'Updated Pizza' });
...
//# sourceMappingURL=menuItem.service.test.js.map