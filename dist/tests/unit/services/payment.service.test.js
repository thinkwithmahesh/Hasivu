...
describe('PaymentService', () => {
    ...
    describe('Service Initialization', () => {
        it('should initialize successfully in test environment', async () => {
            ...
            expect(logger_1.logger.info).toHaveBeenCalledWith('Payment service initialized successfully');
        });
        ...
        it('should handle initialization errors gracefully', async () => {
            jest.spyOn(paymentService, 'isRazorpayAvailable').mockReturnValue(true);
            paymentService['razorpay'] = mockRazorpayInstance;
    ...
    describe('Payment Order Creation', () => {
        ...
        it('should handle Razorpay API errors', async () => {
            ...
            expect(logger_1.logger.error).toHaveBeenCalledWith('Failed to create payment order:', expect.any(Error));
        });
    ...
    describe('Payment Signature Verification', () => {
        ...
        it('should verify valid payment signature', () => {
            const result = paymentService.verifyPaymentSignature('order_123', 'pay_123', 'valid_signature');
        ...
        it('should reject invalid payment signature', () => {
            ...
            const result = paymentService.verifyPaymentSignature('order_123', 'pay_123', 'invalid_signature');
        ...
        it('should handle signature verification errors', () => {
            ...
            const result = paymentService.verifyPaymentSignature('order_123', 'pay_123', 'signature');
            expect(result).toBe(false);
            expect(logger_1.logger.error).toHaveBeenCalledWith('Payment signature verification failed:', expect.any(Error));
        });
    });
    describe('Payment Capture', () => {
        ...
        it('should capture payment successfully', async () => {
            ...
            expect(paymentService.verifyPaymentSignature).toHaveBeenCalledWith('order_razorpay_123', 'pay_razorpay_123', 'valid_signature');
    ...
    describe('Refund Processing', () => {
        ...
        it('should handle Razorpay refund errors', async () => {
            ...
            expect(logger_1.logger.error).toHaveBeenCalledWith('Failed to create refund:', expect.any(Error));
        });
    });
    describe('Subscription Management', () => {
        ...
        describe('createSubscriptionPlan', () => {
            beforeEach(() => {
                ...
                MockedDatabaseService.client.subscriptionPlan.create.mockResolvedValue(mockPlan);
            });
            ...
            it('should use default currency when not provided', async () => {
                ...
                expect(mockRazorpayInstance.plans.create).toHaveBeenCalledWith(expect.objectContaining({
            ...
            it('should handle plan creation errors', async () => {
                ...
                expect(logger_1.logger.error).toHaveBeenCalledWith('Failed to create subscription plan:', expect.any(Error));
            });
        });
        describe('createSubscription', () => {
            beforeEach(() => {
                paymentService['razorpay'] = mockRazorpayInstance;
                MockedDatabaseService.client.user.findUnique.mockResolvedValue(mockUser);
                MockedDatabaseService.client.subscriptionPlan.findUnique.mockResolvedValue(mockPlan);
                mockRazorpayInstance.subscriptions.create.mockResolvedValue(mockRazorpaySubscription);
                MockedDatabaseService.client.paymentSubscription.create.mockResolvedValue(mockSubscription);
            });
            it('should create subscription successfully', async () => {
                ...
                expect(MockedDatabaseService.client.paymentSubscription.create).toHaveBeenCalledWith({
                    data: expect.objectContaining({
            ...
            it('should handle subscription creation errors', async () => {
                ...
                expect(logger_1.logger.error).toHaveBeenCalledWith('Failed to create subscription:', expect.any(Error));
            });
        });
    });
    describe('Webhook Processing', () => {
        ...
        it('should handle subscription.charged webhook', async () => {
            ...
            MockedDatabaseService.client.paymentSubscription.updateMany.mockResolvedValue({ count: 1 });
            const result = await paymentService.handleWebhook(webhookBody, 'expected_signature');
            expect(result.success).toBe(true);
            expect(MockedDatabaseService.client.paymentSubscription.updateMany).toHaveBeenCalledWith({
        ...
        it('should handle unrecognized webhook events', async () => {
            ...
            const result = await paymentService.handleWebhook(webhookBody, 'expected_signature');
            expect(result.success).toBe(true);
            expect(logger_1.logger.warn).toHaveBeenCalledWith('Unhandled webhook event:', 'unknown.event');
        });
        ...
        it('should handle webhook processing errors', async () => {
            ...
            expect(logger_1.logger.error).toHaveBeenCalledWith('Webhook processing failed:', expect.any(Error));
        });
    ...
    describe('Payment Order Retrieval', () => {
        ...
        it('should get payment order from database and cache it', async () => {
            ...
            expect(MockedRedisService.setex).toHaveBeenCalledWith('payment_order:order_razorpay_123', 300, JSON.stringify(mockPaymentOrder));
        });
        ...
        it('should handle database errors gracefully', async () => {
            ...
            expect(result).toBeNull();
            expect(logger_1.logger.error).toHaveBeenCalledWith('Failed to get payment order:', expect.any(Error));
        });
        it('should handle Redis errors gracefully', async () => {
            ...
            expect(logger_1.logger.warn).toHaveBeenCalledWith('Failed to get payment order from cache:', expect.any(Error));
    ...
    describe('Error Handling and Edge Cases', () => {
        it('should handle missing environment configuration', () => {
            ...
            expect(() => new payment_service_1.PaymentService()).not.toThrow();
        });
        ...
        it('should handle network timeouts gracefully', async () => {
            ...
            expect(logger_1.logger.error).toHaveBeenCalledWith('Failed to create payment order:', timeoutError);
        });
        it('should validate webhook signature with timing-safe comparison', async () => {
            ...
            MockedCrypto.timingSafeEqual.mockImplementation(() => {
                ...
                return timingSafeEqualCallCount === 1;
            });
            const webhookBody = JSON.stringify({ event: 'test' });
            const mockHmac = {
                update: jest.fn().mockReturnThis(),
                ...
                payload: {
                    payment: {
                        entity: {
                            id: 'pay_123',
                            status: 'captured',
                            notes: Object.fromEntries(Array.from({ length: 100 }, (_, i) => [`key_${i}`, `value_${i}`]))
                        }
                    }
                }
            };
    ...
    describe('PCI Compliance and Security', () => {
        ...
        it('should use timing-safe comparison for signature verification', () => {
            ...
            expect(MockedCrypto.timingSafeEqual).toHaveBeenCalledWith(expect.any(Buffer), expect.any(Buffer));
        });
