...
(function (DegradationStrategy) {
    ...
    DegradationStrategy["RETRY_WITH_BACKOFF"] = "retry_with_backoff"; // Retry with exponential backoff
...
class GracefulDegradationService {
    ...
    async cacheResult(cacheKey, data, durationMs) {
        try {
            ...
            await this.redis.setex(`fallback:${cacheKey}`, Math.floor(durationMs / 1000), // Convert to seconds
            JSON.stringify(data));
        }
        catch (error) {
