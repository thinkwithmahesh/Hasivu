...
export interface SystemMetrics {
  ...
  memory: {
    used: string; // Used memory in MB
    total: string; // Total memory in MB
...
export interface HealthCheckConfig {
  ...
  checkTimeout: number; // Timeout per service check in ms
  warningThresholds: {
...
export class HealthCheckServiceError extends Error {
  ...
  constructor(message: string, code: string = 'HEALTH_CHECK_ERROR', service?: string, details?: any) {
    super(message);
...
export class HealthCheckService {
  ...
  private constructor() {
    this.config = {
      enableDetailedChecks: (config as any).environment !== 'production' || (config.monitoring as any)?.enableDetailedHealthChecks || false,
      checkTimeout: (config.monitoring as any)?.healthCheckTimeout || 5000,
      warningThresholds: {
        cpuUsage: 70,
        memoryUsage: 75,
        responseTime: 1000
      ...
      services: {
        database: true,
        redis: (config.redis as any)?.enabled || false,
        email: (config.sendgrid as any)?.enabled || false,
        payment: (config.razorpay as any)?.enabled || false
      }
    };
   ...
   */
  private generateMetricAlerts(metrics: SystemMetrics): HealthAlert[] {
    const alerts: HealthAlert[] = [];
   ...
   */
  private async performServiceChecks(): Promise<ServiceHealthStatus[]> {
    const services: ServiceHealthStatus[] = [];
    const checkPromises: Promise<ServiceHealthStatus>[] = [];
    ...
    const results = await Promise.allSettled(
      checkPromises.map(promise => 
        this.withTimeout(promise, this.config.checkTimeout)
      )
    );
  ...
  private async checkDatabaseHealth(): Promise<ServiceHealthStatus> {
    ...
    try {
      ...
      if (typeof (dbService as any).healthCheck === 'function') {
        const healthResult = await (dbService as any).healthCheck();
   ...
   */
  private async checkEmailServiceHealth(): Promise<ServiceHealthStatus> {
    const startTime = Date.now();
   ...
   */
  private async checkPaymentServiceHealth(): Promise<ServiceHealthStatus> {
    const startTime = Date.now();
    ...
    try {
      ...
      if (typeof paymentService.healthCheck === 'function') {
        ...
        return {
          name: 'payment',
          status: (serviceInfo as any).initialized && (serviceInfo as any).keyConfigured ? 'healthy' : 'degraded',
          responseTime,
          lastCheck: Date.now(),
          details: {
            ...serviceInfo,
   ...
   */
  public getHealthHistory(): SystemHealthStatus[] {
   ...
   */
  public getLatestHealthStatus(): SystemHealthStatus | null {
   ...
   */
  public getServiceConfig(): HealthCheckConfig {
    return { ...this.config };
  }
