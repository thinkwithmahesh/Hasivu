...
export const csrfProtection = (config: CSRFConfig = {}) => {
  ...
  return async (req: CSRFRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const csrfToken =
        (req.headers[csrfConfig.customHeaderName!.toLowerCase()] as string) ||
        req.body?.csrfToken ||
      ...
      logger.error('CSRF protection middleware error', undefined, {
...
export const attachCSRFToken = async (
  ...
  try {
    ...
    logger.error('Failed to attach CSRF token', undefined, {
      errorMessage: error instanceof Error ? error.message : 'Unknown error',
      sessionId: req.sessionId,
    });
...
export const doubleSubmitCSRF = (config: CSRFConfig = {}) => {
  ...
  return (req: CSRFRequest, res: Response, next: NextFunction): void => {
    try {
      ...
      const headerToken =
        (req.headers[csrfConfig.customHeaderName!.toLowerCase()] as string) || req.body?.csrfToken;
      ...
      logger.error('Double submit CSRF protection error', undefined, {
        errorMessage: error instanceof Error ? error.message : 'Unknown error',
        method: req.method,
        path: req.path,
      });
...
export const sameSiteCSRF = (req: Request, res: Response, next: NextFunction): void => {
  try {
    ...
    logger.error('SameSite CSRF protection error', undefined, {
      errorMessage: error instanceof Error ? error.message : 'Unknown error',
      method: req.method,
      path: req.path,
    });
...
export const enhancedCSRFProtection = (config: CSRFConfig = {}) => {
  return [sameSiteCSRF, csrfProtection(config), attachCSRFToken];
};
...
export const getCSRFToken = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    logger.error('Failed to generate CSRF token', undefined, {
      errorMessage: error instanceof Error ? error.message : 'Unknown error',
    });
