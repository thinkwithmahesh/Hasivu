...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { cardNumber, studentId, schoolId: requestedSchoolId } = req.body;
    const currentUser = req.user!;
    ...
    const schoolId = currentUser.schoolId || requestedSchoolId;
    if (!schoolId) {
      throw createValidationError('schoolId is required when it is not present in auth context');
    }

    if (currentUser.role === 'school_admin' && currentUser.schoolId !== schoolId) {
      throw new AppError('You can only register cards for your school', 403);
    }

    // Fallback or autogenerated card number
    const generatedId = `RFID-${Date.now().toString(36).toUpperCase()}`;
