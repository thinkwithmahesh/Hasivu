/**
 * HASIVU Platform - Enhanced Order Service with Dependency Injection
 * Service Layer Architecture Enhancement - Phase 2
 * Implements proper dependency injection for testability and maintainability
 */

import { Order, OrderItem, PaymentOrder, Prisma } from '@prisma/client';
import { 
  IOrderRepository,
...
/**
 * Service interfaces (keeping from original service)
 */
...
/**
 * Enhanced Order Service with Dependency Injection
 */
export class EnhancedOrderService {
  private readonly CACHE_TTL = 300; // 5 minutes
  private readonly CART_EXPIRY = 3600; // 1 hour
  private readonly MAX_QUANTITY_PER_ITEM = 10;
  private readonly ORDER_CUTOFF_HOURS = 2; // Hours before delivery
   ...
   */
  public async createOrder(input: CreateOrderInput): Promise<ServiceResponse<Order>> {
    try {
      ...
      const order = await this.container.databaseService.transaction(async (tx) => {
        ...
        await this.container.orderItemRepository.createMany(
          orderItems.map(item => ({
            ...item,
   ...
   */
  public async addToCart(input: AddToCartInput): Promise<ServiceResponse<Cart>> {
    try {
   ...
   */
  public async getCart(studentId: string): Promise<ServiceResponse<Cart | null>> {
    try {
      const cartKey = `cart:${studentId}`;
   ...
   */
  private async validateOrderInput(input: CreateOrderInput): Promise<ServiceResponse<void>> {
