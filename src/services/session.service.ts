...
export class SessionService {
  ...
  private constructor() {
    ...
    this.redis.on('error', error => {
      logger.error('Redis connection error', undefined, {
  ...
  public async createSession(
    ...
    sessionData: Omit<
      SessionData,
      'sessionId' | 'createdAt' | 'lastActivity' | 'expiresAt' | 'isActive'
    >,
    ...
    try {
      ...
      const maxSessions =
        options.maxConcurrentSessions || this.getMaxSessionsForRole(sessionData.role);
   ...
   */
  public async validateSession(sessionId: string, req?: Request): Promise<SessionValidationResult> {
    try {
      if (!sessionId) {
        return {
          valid: false,
      ...
      logger.error('Session validation error', undefined, {
        sessionId,
        errorMessage: error instanceof Error ? error.message : String(error),
      });
      return {
  ...
  public async updateSessionActivity(
    ...
    try {
      ...
      logger.error('Failed to update session activity', undefined, {
        sessionId,
        errorMessage:
  ...
  public async destroySession(sessionId: string): Promise<boolean> {
    try {
      ...
      logger.error('Failed to destroy session', undefined, {
        sessionId,
        errorMessage:
          error instanceof Error
            ? error instanceof Error
  ...
  public async destroyAllUserSessions(userId: string, excludeSessionId?: string): Promise<number> {
    try {
      ...
      logger.error('Failed to destroy user sessions', undefined, {
        userId,
        errorMessage:
          error instanceof Error
            ? error instanceof Error
  ...
  public async validateCSRFToken(sessionId: string, token: string): Promise<boolean> {
    try {
      ...
      logger.error('CSRF validation error', undefined, {
        sessionId,
        errorMessage:
          error instanceof Error
            ? error instanceof Error
  ...
  private async getUserSessions(userId: string): Promise<string[]> {
    try {
      ...
      logger.error('Failed to get user sessions', undefined, {
        userId,
        errorMessage:
          error instanceof Error
            ? error instanceof Error
  ...
  private async addUserSession(userId: string, sessionId: string, ttlMs: number): Promise<void> {
    try {
      ...
      logger.error('Failed to add user session', undefined, {
        userId,
        sessionId,
        errorMessage:
          error instanceof Error
  ...
  private async removeUserSession(userId: string, sessionId: string): Promise<void> {
    try {
      ...
      logger.error('Failed to remove user session', undefined, {
        userId,
        sessionId,
        errorMessage:
          error instanceof Error
  ...
  public async getSessionStats(): Promise<{
    ...
    try {
      ...
      logger.error('Failed to get session stats', undefined, {
        errorMessage:
          error instanceof Error
            ? error instanceof Error
              ? error.message
  ...
  public async healthCheck(): Promise<{ status: 'healthy' | 'unhealthy'; details: any }> {
    try {
      ...
      await this.redis.setex(
        `${this.sessionPrefix}${testSessionId}`,
        60,
