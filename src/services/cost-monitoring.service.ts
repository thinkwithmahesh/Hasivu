 ...
 * @since 2024
 */
import { CloudWatch, CostExplorer, Budgets } from 'aws-sdk';
...
/**
 * Cost optimization recommendations interface
 */
...
export class CostMonitoringService {
  ...
  constructor() {
    ...
    this.costExplorer = new CostExplorer({ region: 'us-east-1' }); // Cost Explorer is only available in us-east-1
    this.cloudwatch = new CloudWatch({ region: process.env.AWS_REGION });
    this.budgets = new Budgets({ region: 'us-east-1' }); // Budgets is only available in us-east-1
   ...
   */
  async generateCostReport(period: string = 'daily'): Promise<CostReport> {
    const startTime = Date.now();
    const reportId = `cost_report_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    logger.info(`Generating cost report for period: ${period}`, { reportId });
    ...
    try {
      ...
      logger.info(`Cost report generated in ${processingTime}ms`, { reportId, totalCost });

      return report;
    } catch (error) {
      logger.error('Failed to generate cost report:', error);
      throw new Error(`Cost report generation failed: ${(error as Error).message}`);
    }
  }
   ...
   */
  private async getCostBreakdown(startDate: string, endDate: string): Promise<CostBreakdown[]> {
    try {
      const params = {
        TimePeriod: {
          Start: startDate,
   ...
   */
  private async calculateServiceTrend(serviceName: string, startDate: string, endDate: string): Promise<'increasing' | 'decreasing' | 'stable'> {
    try {
      ...
      logger.warn(`Failed to calculate trend for service ${serviceName}:`, error);
      return 'stable';
    }
  }
   ...
   */
  private async getCostTrends(startDate: string, endDate: string): Promise<CostTrends> {
    try {
      ...
      const dailyCosts = result.ResultsByTime.map(item => 
        parseFloat(item.Total?.BlendedCost?.Amount || '0')
      );
   ...
   * Get budget statuses
   */
  private async getBudgetStatuses(): Promise<BudgetStatus[]> {
    try {
      ...
      logger.error('Failed to get budget statuses:', error);
      return [];
    }
  }
   ...
   */
  private async getCostEfficiencyMetrics(): Promise<CostEfficiencyMetrics> {
    try {
      ...
      logger.error('Failed to get cost efficiency metrics:', error);
      return {
        rightsizingOpportunities: [],
        unusedResources: [],
        reservedInstanceUtilization: {
   ...
   */
  private async detectCostAnomalies(startDate: string, endDate: string): Promise<CostAnomaly[]> {
    try {
      const params = {
        DateInterval: {
          StartDate: startDate,
      ...
      if (!process.env.COST_ANOMALY_MONITOR_ARN) {
        logger.warn('Cost anomaly monitor ARN not configured');
        return [];
      }
      ...
      if (result.Anomalies) {
        for (const anomaly of result.Anomalies) {
          anomalies.push({
            ...
            dimension: (anomaly as any).Dimension || '',
   ...
   */
  private generateCostAlerts(budgets: BudgetStatus[], anomalies: CostAnomaly[], trends: CostTrends): CostAlert[] {
  ...
  private async storeCostReport(report: CostReport): Promise<void> {
    try {
      ...
      logger.error('Failed to store cost report:', error);
    }
  }
   ...
   */
  public startContinuousMonitoring(intervalMs: number = 3600000): void { // Default: 1 hour
    ...
    this.monitoringInterval = setInterval(async () => {
      try {
        ...
        logger.error('Continuous cost monitoring error:', error);
      }
    }, intervalMs);

    logger.info(`Started continuous cost monitoring with ${intervalMs}ms interval`);
  }
  ...
  public stopContinuousMonitoring(): void {
    if (this.monitoringInterval) {
      ...
      logger.info('Stopped continuous cost monitoring');
    }
  }
   ...
   */
  private generateCostForecast(historicalCosts: number[]): CostTrends['forecast'] {
   ...
   * Analyze seasonality
   */
  private analyzeSeasonality(historicalCosts: number[]): CostTrends['seasonality'] {
    ...
    let pattern: 'seasonal' | 'stable' | 'volatile';
   ...
   */
  private getDefaultTrends(): CostTrends {
    return {
      dailyGrowth: 0,
      weeklyGrowth: 0,
      monthlyGrowth: 0,
   ...
   */
  private async getBudgetActualSpend(budgetName: string): Promise<number> {
    try {
      ...
      logger.warn(`Failed to get actual spend for budget ${budgetName}:`, error);
      return 0;
    }
  }
   ...
   * Calculate days remaining in budget period
   */
  private calculateDaysRemaining(timePeriod: any): number {
    try {
      if (timePeriod?.End) {
