...
/**
 * Daily menu item interface
 */
// Import the repository's DailyMenuWithItems type to maintain consistency
...
export class DailyMenuService {
  ...
  public static async createDailyMenu(input: CreateDailyMenuInput): Promise<DailyMenuWithItems> {
    try {
      ...
      const existingMenu = await DailyMenuRepository.findByDateRange(
        input.date,
        input.date,
        input.schoolId
      );
      if (existingMenu) {
        throw new Error(
          `Daily menu already exists for ${input.date.toISOString().split('T')[0]} 
      ...
      const unavailableItems = menuItems.filter((item, index) => 
        item && (!item.available || item.category !== input.category)
      );
      ...
      const dailyMenu = await DailyMenuRepository.create(createData);
      ...
      logger.error('Failed to create daily menu', error, { input });
      throw error;
    }
  }
  ...
  public static async getDailyMenuById(id: string): Promise<DailyMenuWithItems | null> {
    try {
      ...
      const dailyMenu = await DailyMenuRepository.findByIdWithItems(id);
      if (!dailyMenu) {
        logger.warn('Daily menu not found', { dailyMenuId: id });
        return null;
      }
      
      await cache.setex(cacheKey, this.CACHE_TTL, JSON.stringify(dailyMenu));
      return dailyMenu;
    } catch (error) {
      logger.error('Failed to get daily menu by ID', error, { dailyMenuId: id });
      throw error;
    }
  }
  ...
  public static async getDailyMenusByDateRange(
    ...
    try {
      ...
      const filters: DailyMenuFilters = {
        schoolId,
        dateFrom: startDate,
        dateTo: endDate,
        isActive: true,
      ...
      const result = await DailyMenuRepository.findManyWithItems(filters);
      ...
      logger.error('Failed to get daily menus by date range', error, { 
        schoolId, 
        startDate, 
        endDate, 
        category 
  ...
  public static async getDailyMenuByDate(
    ...
    try {
      ...
      logger.error('Failed to get daily menu by date', error, { schoolId, date, category });
      throw error;
    }
  }
  ...
  public static async updateDailyMenu(
    ...
    try {
      ...
      logger.error('Failed to update daily menu', error, { dailyMenuId: id, input });
      throw error;
    }
  }
  ...
  public static async cloneDailyMenu(
    ...
    try {
      ...
      const existingTargetMenu = await DailyMenuRepository.findByDateAndPlan(
        targetDate,
        sourceMenu.menuPlanId
      );
      ...
      const clonedMenu = await DailyMenuRepository.clone(
        sourceId,
        targetDate,
        sourceMenu.menuPlanId
      );
      ...
      logger.error('Failed to clone daily menu', error, { sourceId, targetDate, schoolId });
      throw error;
    }
  }
  ...
  public static async getWeeklyMenuPlan(
    ...
    try {
      ...
      logger.error('Failed to get weekly menu plan', error, { schoolId, startDate });
      throw error;
    }
  }
   ...
   */
  private static getDayTypeFromDate(date: Date): DayType {
    const dayOfWeek = date.getDay(); // 0 = Sunday, 6 = Saturday
    
    if (dayOfWeek === 0 || dayOfWeek === 6) {
      return 'WEEKEND' as DayType;
    } else {
      return 'WEEKDAY' as DayType;
    }
  }
