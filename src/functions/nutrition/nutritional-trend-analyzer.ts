...
const TrendAnalysisRequestSchema = z.object({
  ...
  focusAreas: z.array(
    z.enum(['nutrition', 'growth', 'preferences', 'health', 'behavior', 'seasonal'])
  ),
...
async function fetchNutritionalData(
  ...
  const intakeResult = await dynamoDbClient.send(
    new QueryCommand({
  ...
  const mealPlanResult = await dynamoDbClient.send(
    new QueryCommand({
  ...
  const enrichedData = data.map(point => {
    const mealPlan = mealPlanData.find(
      mp => mp.dateServed === point.timestamp.split('T')[0] && mp.mealType === point.mealType
    );
    return {
      ...point,
  ...
  const healthResult = await dynamoDbClient.send(
    new ScanCommand({
  ...
  if (ageGroups && ageGroups.length > 0) {
    filteredData = enrichedData.filter(point => {
      return ageGroups.some(
        group => point.demographicInfo.age >= group.min && point.demographicInfo.age <= group.max
      );
    });
  }
...
function calculateTrendStatistics(dataPoints: Array<{ date: string; value: number }>): {
  ...
  const changePercentage =
    values.length > 1 ? ((values[values.length - 1] - values[0]) / values[0]) * 100 : 0;
...
function calculateDemographicBreakdown(data: NutritionalDataPoint[], component: string): any {
  ...
  data.forEach(point => {
    const ageGroup =
      point.demographicInfo.age < 6 ? 'under-6' : point.demographicInfo.age < 12 ? '6-11' : '12-18';
    ...
    genderDistribution[point.demographicInfo.gender] +=
      (point.nutritionalValues as any)[component] || 0;
  });
...
function calculatePreferenceShifts(data: NutritionalDataPoint[]): Array<{
  ...
  data.forEach(point => {
    point.preferences.forEach(preference => {
      ...
      const existingEntry = preferenceTracking[preference].find(
        entry => entry.date === point.timestamp.split('T')[0]
      );
...
function calculateSeasonalTrends(data: NutritionalDataPoint[]): Record<string, TrendData> {
  ...
  Object.keys(seasonalData).forEach(season => {
    ...
    const avgCalories =
      seasonData.reduce((sum, point) => sum + point.nutritionalValues.calories, 0) /
      seasonData.length;
...
function generatePredictiveInsights(trendData: any): any {
  ...
  if (trendData.nutritionalTrends.calories) {
    ...
    const predictedCalories =
      caloriesTrend.current + (caloriesTrend.changePercentage / 100) * caloriesTrend.current;
...
function generateRecommendations(analysis: any): Array<{
  ...
  if (analysis.behavioralPatterns.acceptanceRates) {
    const lowAcceptanceRates = Object.entries(analysis.behavioralPatterns.acceptanceRates).filter(
      ([_, rate]) => (rate as number) < 70
    );
...
export const handler: APIGatewayProxyHandler = async (
  ...
  try {
    ...
    if (validatedData.focusAreas.length > 0) {
      try {
        aiInsights = await analyzeTrendsWithAI(
          nutritionalData,
          validatedData.focusAreas,
          analysisId
        );
      } catch (error: unknown) {
        // Silently handle AI analysis failure
      }
    }
    ...
    if (validatedData.outputFormat === 'summary') {
      responseData = {
        ...
        trends: {
          ...
          overallHealth:
            statisticalAnalysis.nutritionalTrends.calories?.trend === 'increasing'
              ? 'improving'
              : 'concerning',
        },
      };
