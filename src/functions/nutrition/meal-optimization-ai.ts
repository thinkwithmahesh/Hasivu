...
const mealOptimizationRequestSchema = z.object({
  ...
  targets: z.object({
    ...
    nutritionalBalance: z.object({
      ...
      fatRatio: z.number().min(0).max(1).default(0.30)
    })
  }),
  ...
  userProfile: z.object({
    ...
    healthGoals: z.array(z.enum(['weight_loss', 'muscle_gain', 'maintenance', 'endurance', 'strength'])),
...
async function getNutritionalData(foodIds: string[]): Promise<Record<string, any>> {
  ...
  try {
    ...
    LoggerService.getInstance().error('Failed to fetch nutritional data', { error, foodIds });
...
Focus on practical, achievable improvements that align with the user's constraints and preferences.`;
    ...
    LoggerService.getInstance().error('AI meal optimization failed', { error });
...
function processAIOptimization(aiResponse: any, originalMealPlan: any): OptimizedMealPlan {
  return {
    meals: aiResponse.optimized_meals?.map((meal: any) => ({
 ...
 */
function createFallbackOptimization(inputData: any, constraints: any, targets: any): OptimizedMealPlan {
...
export const mealOptimizationHandler = async (
  ...
  try {
    ...
    await storeOptimizationResult(authenticatedUser.id, optimizationRequest, optimizationResult);
    ...
    logger.error('AI meal optimization failed', {
      requestId,
      error: error.message,
      stack: error.stack
    });
    ...
    if (error.name === 'ZodError') {
      return createErrorResponse('Invalid optimization request data', 400, 'VALIDATION_ERROR');
    }
