...
const dietaryProfileSchema = z.object({
  ...
  preferences: z.object({
    dietType: z.enum(['vegetarian', 'vegan', 'keto', 'paleo', 'mediterranean', 'omnivore', 'pescatarian']),
    cuisinePreferences: z.array(z.string()),
    mealFrequency: z.number().int().min(1).max(6).default(3),
    calorieTarget: z.number().positive().optional()
  }),
  goals: z.object({
    primary: z.enum(['weight_loss', 'muscle_gain', 'maintenance', 'endurance', 'strength', 'general_health']),
  ...
  healthConditionSupport: z.array(z.object({
    condition: z.string(),
...
Return structured JSON response with meal plans, insights, and actionable recommendations.`;
    ...
    LoggerService.getInstance().error('AI recommendation generation failed', { error });
 ...
 */
function processAIRecommendations(aiResponse: any, profile: any, nutritionalNeeds: NutritionalMetrics): RecommendationResponse {
 ...
 */
function createDailyMealPlan(dayIndex: number, nutritionalNeeds: NutritionalMetrics, profile: any): DailyMealPlan {
 ...
 */
function createMealRecommendation(mealType: string, targetCalories: number, profile: any): MealRecommendation {
 ...
 */
function createNutritionalAnalysis(nutritionalNeeds: NutritionalMetrics, profile: any): NutritionalAnalysis {
  return {
    weeklyTotals: {
    ...
    healthConditionConsiderations: profile.healthConditions?.map((condition: any) => 
 ...
 */
function createFallbackRecommendations(profile: any, nutritionalNeeds: NutritionalMetrics): RecommendationResponse {
  const weeklyMealPlan: WeeklyMealPlan = {
    id: `plan_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    weekStarting: new Date().toISOString().split('T')[0],
    meals: Array.from({ length: 7 }, (_, i) => createDailyMealPlan(i, nutritionalNeeds, profile)),
...
export const dietaryRecommendationHandler = async (
  ...
  try {
    ...
    if (error.name === 'ZodError') {
      return createErrorResponse('Invalid dietary profile data', 400, 'VALIDATION_ERROR');
    }
