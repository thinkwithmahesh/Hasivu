 ...
 */
async function runDatabaseTests(): Promise<DatabaseTest[]> {
  ...
  try {
    const startTime = Date.now();
    await DatabaseService.client.$queryRaw`SELECT version() as version`;
    tests.push({
      name: 'version_check',
      status: 'passed',
      responseTime: Date.now() - startTime
  ...
  try {
    ...
    const tableExists = await DatabaseService.client.$queryRaw`
      SELECT EXISTS (
    ...
    if (tableExists) {
      // Simple count query
      await DatabaseService.client.$queryRaw`SELECT COUNT(*) FROM users LIMIT 1`;
    }
