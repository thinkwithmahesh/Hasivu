/**
 * HASIVU Platform - Create Daily Menu Lambda Function
 * Handles: POST /menus/daily
 * Implements Story 3.1: Menu Planning & Management - Daily Menu Creation
 * Production-ready with comprehensive validation, slot management, and audit logging
 */
...
/**
 * Daily menu creation interface
 */
interface DailyMenuCreateData {
  menuPlanId: string;
  date: string; // YYYY-MM-DD format
  dayType: 'regular' | 'special' | 'holiday' | 'event';
  name?: string;
  description?: string;
  slots: MenuSlotData[];
  isActive?: boolean;
...
/**
 * Menu slot data interface
 */
interface MenuSlotData {
...
/**
 * Validate menu plan exists and user has permission
 */
async function validateMenuPlanAccess(menuPlanId: string, userId: string): Promise<any> {
  ...
  // Check user access to the school
  ...
  // Check if user has permission to create daily menus
...
/**
 * Validate date is within menu plan range and not duplicate
 */
async function validateDateWithinPlan(date: Date, menuPlan: any): Promise<void> {
  const targetDate = new Date(date);
  const startDate = new Date(menuPlan.startDate);
  const endDate = new Date(menuPlan.endDate);
  
  // Check if date is within menu plan range
 ...
 * Validate day type
 */
function validateDayType(dayType: string): void {
 ...
 */
async function validateMenuItems(menuItemIds: string[], schoolId: string): Promise<any[]> {
...
/**
 * Validate menu slots data
 */
async function validateMenuSlots(slots: MenuSlotData[], schoolId: string): Promise<void> {
...
/**
 * Create daily menu with slots in database
 */
async function createDailyMenuInDatabase(data: DailyMenuCreateData, userId: string): Promise<DailyMenuResponse> {
  const database = DatabaseService.getInstance();
  ...
  try {
    ...
    for (let i = 0; i < data.slots.length; i++) {
      ...
      const slotResult = await database.query(slotQuery, [
        ...
        slot.priority || (i + 1) * 10, // Default priority based on order
        JSON.stringify(slot.metadata || {})
      ]);
      
      const slotId = slotResult.rows[0].id;
      
      // Link menu items to slot if provided
      if (slot.menuItemIds && slot.menuItemIds.length > 0) {
    ...
    // Get the complete created daily menu with details
...
/**
 * Get daily menu with all details
 */
async function getDailyMenuWithDetails(dailyMenuId: string): Promise<DailyMenuResponse> {
  const database = DatabaseService.getInstance();
  
  // Get daily menu with menu plan and school details
  const dailyMenuQuery = `
    SELECT 
  ...
  // Group slots and their menu items
...
/**
 * Create Daily Menu Lambda Function Handler
 * Creates a new daily menu within a menu plan with comprehensive validation
 */
export const createDailyMenuHandler = async (
  ...
  try {
    ...
    let requestData: DailyMenuCreateData;
    try {
    ...
    if (!requestData.menuPlanId) {
      return createErrorResponse('Menu plan ID is required', 400, 'MISSING_MENU_PLAN_ID');
    }
    
    try {
      validationService.validateUUID(requestData.menuPlanId, 'Menu plan ID');
    } catch (error: any) {
      return createErrorResponse('Invalid menu plan ID format', 400, 'INVALID_MENU_PLAN_ID');
    }
    
    if (!requestData.date) {
      return createErrorResponse('Date is required', 400, 'MISSING_DATE');
    }
    
    // Validate date format (YYYY-MM-DD)
    if (!/^\d{4}-\d{2}-\d{2}$/.test(requestData.date)) {
      return createErrorResponse('Invalid date format. Use YYYY-MM-DD', 400, 'INVALID_DATE_FORMAT');
    }
    
    if (!requestData.dayType) {
      return createErrorResponse('Day type is required', 400, 'MISSING_DAY_TYPE');
    }
    
    if (!requestData.slots || !Array.isArray(requestData.slots)) {
      return createErrorResponse('Slots array is required', 400, 'MISSING_SLOTS');
    }
    ...
    // Validate menu plan access and permissions
    ...
    return handleError(error, 'Failed to create daily menu');
  }
};