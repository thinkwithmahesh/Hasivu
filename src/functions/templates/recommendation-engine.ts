...
function determineUserSegment(userProfile: UserProfile): string {
  const avgEngagement = userProfile.engagementPatterns.reduce(
    (acc, pattern) => acc + pattern.engagementRate, 0
...
function calculateEngagementScore(userProfile: UserProfile): number {
  const patternScore = userProfile.engagementPatterns.reduce(
    (acc, pattern) => acc + pattern.engagementRate, 0
  ) / userProfile.engagementPatterns.length || 0;
  
  const contentScore = userProfile.contentPreferences.reduce(
    (acc, content) => acc + content.engagementRate, 0
