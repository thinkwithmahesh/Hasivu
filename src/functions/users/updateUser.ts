...
export const updateUserHandler = async (
  ...
  try {
    ...
    const validation = await ValidationService.validateObject(filteredUpdateData, updateUserSchema);
...
function filterUpdateDataByPermissions(
  ...
  const filteredData: UpdateUserRequest = { ...updateData };
  ...
  if (requestingUser.id === targetUser.id) {
    const allowedFields = ['firstName', 'lastName', 'metadata'];
    const restrictedUpdate: UpdateUserRequest = {};
  ...
  if (requestingUser.role === 'parent' && targetUser.parentId === requestingUser.id) {
    const allowedFields = ['firstName', 'lastName', 'metadata'];
    const parentUpdate: UpdateUserRequest = {};
    for (const field of allowedFields) {
      if (updateData[field as keyof UpdateUserRequest] !== undefined) {
