/**
 * HASIVU Platform - Search Menu Items Lambda Function
 * Handles: GET /menu/search
 * Implements Story 2.1: Product Catalog Foundation - Advanced Menu Item Search
 * Production-ready with comprehensive search capabilities and performance optimization
 */
...
/**
 * Search filters interface with comprehensive options
 */
interface SearchFilters {
  ...
  availableDay?: string;
  preparationTimeMax?: number; // in minutes
...
/**
 * Validate and sanitize search term to prevent injection attacks
 */
function validateAndSanitizeSearchTerm(searchTerm: string): string {
 ...
 */
function generateCacheKey(filters: SearchFilters, page: number, limit: number): string {
 ...
 */
function buildSearchQuery(filters: SearchFilters, limit: number, offset: number): {
  query: string;
  values: any[];
} {
 ...
 */
async function getSearchResultCount(filters: SearchFilters): Promise<number> {
  ...
  // Same filtering logic as main query but for count
  const searchPattern = `%${filters.searchTerm.toLowerCase()}%`;
  whereConditions.push(`(
    LOWER(name) LIKE $${paramCounter} OR 
    LOWER(description) LIKE $${paramCounter} OR
...
/**
 * Search Menu Items Lambda Function Handler
 * Advanced search with full-text capabilities, filtering, and caching
 */
export const searchMenuItemsHandler = async (
  ...
  try {
    ...
    if (!rawSearchTerm) {
      ...
      return createErrorResponse('Search term is required', 400, 'MISSING_SEARCH_TERM');
    }
    ...
    try {
      ...
      return createErrorResponse(error.message, 400, 'INVALID_SEARCH_TERM');
    }
    
    // Parse pagination parameters
    const page = Math.max(1, parseInt(queryParams.page || '1'));
    const limit = Math.min(50, Math.max(1, parseInt(queryParams.limit || '20'))); // Max 50 items
    ...
    try {
      ...
      if (cachedResult) {
        ...
        return createSuccessResponse(JSON.parse(cachedResult));
      }
    ...
    // Format results
    const results: SearchResultItem[] = searchResults.rows.map(row => ({
      id: row.id,
      name: row.name,
    ...
    return handleError(error, 'Failed to search menu items');
  }
};