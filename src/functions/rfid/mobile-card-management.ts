...
interface ReplacementInfo {
  id: string;
  requestDate: Date;
  status: 'pending' | 'approved' | 'issued' | 'completed';
  reason: string;
...
interface RfidIssueReportRequest {
  studentId: string;
  issueType: 'lost' | 'damaged' | 'not_working' | 'other';
  description: string;
 ...
 */
export async function reportRfidIssue(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
    // Authenticate user
    const authResult = await authenticateJWT(event);
    if (!authResult.success) {
