...
const CustomizationRequestSchema = z.object({
  action: z.enum(['get_customization', 'update_layout', 'manage_widgets', 'update_theme', 'reset_to_defaults']),
 ...
 */
async function updateLayoutConfiguration(userId: string, request: CustomizationRequest): Promise<CustomizationResponse> {
  ...
  if (updatedLayout.type !== currentConfig.layout.type) {
    currentConfig.widgets = await reorganizeWidgetsForLayout(
 ...
 */
async function manageWidgets(userId: string, request: CustomizationRequest): Promise<CustomizationResponse> {
  const currentConfig = await getCustomizationConfig(userId);
  ...
  switch (operation) {
    case 'add':
      ...
      currentConfig.widgets.push(await createWidgetConfiguration(widget_config, currentConfig.layout));
      break;
    ...
    case 'update':
      if (!widget_id || !widget_config) throw new Error('Widget ID and configuration required for update operation');
 ...
 */
async function updateThemeConfiguration(userId: string, request: CustomizationRequest): Promise<CustomizationResponse> {
  const currentConfig = await getCustomizationConfig(userId);
...
function getWidgetCategory(widgetType: string): string {
  const widget = CUSTOMIZATION_CONFIG.widgets[widgetType as keyof typeof CUSTOMIZATION_CONFIG.widgets];
