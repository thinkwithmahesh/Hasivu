...
export const handler = async (
  ...
  try {
    ...
    logger.error('School hierarchy manager request failed', error as Error, { requestId });
...
async function getHierarchyNode(
  ...
  try {
    ...
    if (!node.length) {
      return createErrorResponse('NODE_NOT_FOUND', 'Hierarchy node not found', 404);
    }
...
async function createHierarchyNode(
  ...
  try {
    ...
    if (!nodeData.name || !nodeData.type || !nodeData.code) {
      return createErrorResponse('VALIDATION_ERROR', 'Name, type, and code are required', 400);
    }
    ...
    if (nodeData.parentId) {
      ...
      if (!parent.length) {
        return createErrorResponse('PARENT_NOT_FOUND', 'Parent node not found', 404);
      }
    ...
    if (existing.length) {
      return createErrorResponse('DUPLICATE_CODE', 'Node with this code already exists', 409);
    }
...
async function moveHierarchyNode(
  ...
  try {
    ...
    if (newParentId === nodeId) {
      return createErrorResponse('INVALID_MOVE', 'Cannot move node to itself', 400);
    }
    ...
    if (newParentId) {
      ...
      if (!parent.length) {
        return createErrorResponse('PARENT_NOT_FOUND', 'New parent node not found', 404);
      }
...
async function assignRole(
  ...
  try {
    ...
    if (!userId || !roleId || !scopeType || !scopeId) {
      return createErrorResponse(
        'VALIDATION_ERROR',
        'userId, roleId, scopeType, and scopeId are required',
        400
      );
    }
...
async function bulkAssignRoles(
  ...
  try {
    ...
    if (!Array.isArray(assignments) || assignments.length === 0) {
      return createErrorResponse('VALIDATION_ERROR', 'Assignments array is required', 400);
    }
...
async function getPermissionMatrix(
  ...
  try {
    ...
    if (roleId) {
      ...
      if (!role.length) {
        return createErrorResponse('ROLE_NOT_FOUND', 'Role not found', 404);
      }
...
async function deleteHierarchyNode(
  ...
  try {
    ...
    if (children.length > 0) {
      return createErrorResponse('NODE_HAS_CHILDREN', 'Cannot delete node with children', 400);
    }
    ...
    if (!result.length) {
      return createErrorResponse(
        'NODE_NOT_FOUND',
        'Hierarchy node not found or access denied',
        404
      );
    }
...
async function updateRoleAssignment(
  ...
  try {
    ...
    if (!assignmentId) {
      return createErrorResponse('VALIDATION_ERROR', 'Assignment ID is required', 400);
    }
    ...
    if (!result.length) {
      return createErrorResponse('ASSIGNMENT_NOT_FOUND', 'Role assignment not found', 404);
    }
