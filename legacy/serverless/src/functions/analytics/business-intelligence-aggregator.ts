...
import { z } from 'zod';
import { assertSafePostgresRawSql } from '../../utils/sql-raw-guards';
...
class BusinessIntelligenceAggregator {
  ...
  private async executeAggregationQuery(
    ...
    if (measures.includes('revenue') || measures.includes('orders')) {
      ...
      assertSafePostgresRawSql(query, params);
  ...
  private buildOptimizedAggregationQuery(
    ...
    let placeholder = 1;
    const nextPh = (): string => `$${placeholder++}`;
    ...
    // Build WHERE clause (PostgreSQL $n placeholders — must match getQueryParameters order)
    whereParts.push(
      `p.created_at >= ${nextPh()}::timestamptz`,
      `p.created_at <= ${nextPh()}::timestamptz`,
      "p.status = 'completed'"
    );

    if (filters.schoolId) {
      whereParts.push(`u.school_id = ${nextPh()}::uuid`);
    }

    if (filters.minAmount !== undefined && filters.minAmount !== null) {
      whereParts.push(`p.amount >= ${nextPh()}::double precision`);
    }
   ...
   */
  /** PostgreSQL expressions for time bucketing (replaces legacy SQLite strftime). */
  private getTimeFormatSQL(granularity: string): string {
    switch (granularity) {
      case 'hour':
        return "date_trunc('hour', p.created_at)";
      case 'day':
        return "date_trunc('day', p.created_at)";
      case 'week':
        return "date_trunc('week', p.created_at)";
      case 'month':
        return "date_trunc('month', p.created_at)";
      case 'quarter':
        return "date_trunc('quarter', p.created_at)";
      case 'year':
        return "date_trunc('year', p.created_at)";
      default:
        return "date_trunc('day', p.created_at)";
    }
  }
