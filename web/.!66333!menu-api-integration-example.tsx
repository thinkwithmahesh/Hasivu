...
export default function MenuAPIExample() {
  ...
  const loadMenuItems = async (filters: {
    category?: string;
    search?: string;
    ageGroup?: string;
  } = {}) => {
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                {item.dietary.map(diet => (
                  <span key={diet} className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">
                    {diet}
                  </span>
                ))}
              </div>
              ...
              {item.schoolSpecific && (
                <div className="text-sm">
                  ...
                  <span className="ml-2 font-medium">Age Groups:</span> {item.schoolSpecific.ageGroup.join(', ')}
                </div>
              )}
