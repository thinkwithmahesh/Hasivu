...
export const PaymentSecurityDashboard: React.FC<PaymentSecurityDashboardProps> = ({
  ...
  const loadSecurityMetrics = async (showRefreshing = false) => {
    try {
      ...
      setMetrics(null);
      setError(error instanceof Error ? error.message : 'Failed to load security metrics');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleSecurityValidation = async () => {
    try {
      await loadSecurityMetrics(true);
    } catch (error) {
  ...
  return (
    <div className={cn('w-full space-y-6', className)}>
      ...
      {!metrics && !error && (
        <Alert>
          <AlertDescription>Payment security metrics are not available yet.</AlertDescription>
        </Alert>
      )}
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Alert>
                ...
                <AlertDescription>
                  Your payment system is PCI DSS Level {metrics?.pciCompliance.level} compliant.
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <Badge
                    variant={
                      metrics?.vulnerabilityScan.status === 'passed' ? 'default' : 'destructive'
                    }
              ...
              <Alert>
                ...
                <AlertDescription>
                  Regular vulnerability scans ensure your payment system remains secure. All
