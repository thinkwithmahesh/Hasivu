...
export const PinchZoomImage: React.FC<PinchZoomImageProps> = ({
  ...
  const { pinchState, handlers: pinchHandlers } = usePinchGesture(handlePinch, handlePinchEnd, {
    threshold: 0.05,
  });

  // Pan gesture for when zoomed in
  const handleSwipe = useCallback(
    (direction: any, velocity: number) => {
      if (scale <= 1) return;
      ...
      setPosition(prev => ({
        x: Math.max(
          -maxX,
          Math.min(maxX, prev.x + (direction === 'right' ? 20 : direction === 'left' ? -20 : 0))
        ),
        y: Math.max(
          -maxY,
          Math.min(maxY, prev.y + (direction === 'down' ? 20 : direction === 'up' ? -20 : 0))
        ),
      }));
    },
    [scale]
...
export const LongPressMenu: React.FC<LongPressMenuProps> = ({ children, menuItems, className }) => {
