...
const BULK_DISCOUNT_TIERS: BulkDiscountTier[] = [
  ...
  { minQuantity: 5, discount: 0.1, description: '10% off for 5+ items' },
...
export function QuantitySelector({
  ...
  const priceBreakdown = useMemo((): PriceBreakdown => {
    ...
    const applicableDiscount = BULK_DISCOUNT_TIERS.filter(
  ...
  const currentDiscountTier = useMemo(() => {
    return BULK_DISCOUNT_TIERS.filter(tier => sliderValue[0] >= tier.minQuantity).pop();
  }, [sliderValue]);

  // Get next discount tier
  const nextDiscountTier = useMemo(() => {
    return BULK_DISCOUNT_TIERS.find(tier => sliderValue[0] < tier.minQuantity);
  }, [sliderValue]);
    ...
    <Card className={className}>
      ...
      <CardContent className={`${sizeClasses.card} pt-0 space-y-4`}>
        ...
        {showBulkDiscounts && quantity > 0 && (
          <div className="space-y-2">
            ...
            {nextDiscountTier && !currentDiscountTier && (
              <div className="flex items-center gap-2 p-2 bg-[var(--hasivu-primary)]/5 rounded text-[var(--hasivu-primary-dark)] text-xs">
                <Info className="w-3 h-3" />
                <span>
                  Add {nextDiscountTier.minQuantity - quantity} more for{' '}
                  {nextDiscountTier.discount * 100}% off
                </span>
              </div>
            )}
          </div>
              ...
              <div className="space-y-1 text-xs">
                ...
                <div className="border-t pt-1">
                  <div className="flex justify-between font-medium">
                    <span>Total:</span>
                    <span
                      className={`${sizeClasses.price} ${isInsufficientFunds ? 'text-red-600' : 'text-primary-600'}`}
                    >
                ...
                {priceBreakdown.savingsAmount > 0 && (
                  <div className="flex justify-between text-green-600 font-medium">
                    <span>You Save:</span>
                    <span>
                      ₹{priceBreakdown.savingsAmount.toFixed(2)} (
                      {priceBreakdown.savingsPercentage.toFixed(1)}%)
                    </span>
                  </div>
                )}
              </div>
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <span
            className={`font-medium ${student.walletBalance < priceBreakdown.total ? 'text-red-600' : 'text-green-600'}`}
          >
        ...
        {size !== 'sm' && (
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 5]
                .filter(q => q <= meal.maxQuantityPerStudent)
                .map(popularQty => (
                  <Button
                    key={popularQty}
                    variant="outline"
          ...
          <HoverCard>
            <HoverCardTrigger asChild>
              <div className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer hover:text-primary-600">
                ...
                <span>
                  Nutritional impact for {quantity} item{quantity > 1 ? 's' : ''}
                </span>
              </div>
            </HoverCardTrigger>
