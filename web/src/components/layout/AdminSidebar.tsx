...
export function AdminSidebar({ isCollapsed, onToggleCollapse }: AdminSidebarProps) {
  ...
  return (
    ...
    >
      <div className="h-[60px] flex items-center justify-between px-4 border-b border-pm-neutral-100 shrink-0">
        <AnimatePresence mode="popLayout">
          {!isCollapsed && (
            <motion.div
              ...
              className="font-ui text-[18px] text-pm-primary-600 truncate font-bold"
            >
              Hasivu Admin
            </motion.div>
          )}
        ...
        <button
          ...
          className={`min-h-touch-target min-w-touch-target flex items-center justify-center rounded-lg text-pm-text-tertiary hover:bg-pm-neutral-100 hover:text-pm-text-primary ${isCollapsed ? 'mx-auto' : ''}`}
            ...
            {group.items.map(item => {
              ...
              return (
                <Link
                  key={item.id}
                  href={item.href}
                  className={`relative flex items-center min-h-touch-target px-3 rounded-lg font-ui font-bold text-[14px] overflow-hidden group ${
                    isActive
                      ? 'text-pm-primary-800'
      ...
      <div className="p-4 border-t border-pm-neutral-100 mt-auto">
        <Link
          href="/auth/login"
          className={`flex items-center min-h-touch-target px-3 rounded-lg font-ui font-bold text-[14px] text-pm-semantic-danger hover:bg-red-50 ${isCollapsed ? 'justify-center' : ''}`}
        >
