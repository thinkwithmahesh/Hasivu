...
function generateBreadcrumbs(pathname: string): BreadcrumbItem[] {
  ...
  segments.forEach((segment, index) => {
    ...
    // Convert segment to readable label (unused, kept for reference)
    const _label = segment
      .split('-')
...
function formatBreadcrumbLabel(segment: string, _fullPath: string): string {
...
export function AdminBreadcrumbs({ className }: { className?: string }) {
  return <BreadcrumbNav className={cn('mb-4', className)} showHome={true} />;
}
