...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!upstream.ok) {
      return NextResponse.json(upstreamError(data, 'Failed to fetch purchase orders'), {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const upstream = await forwardToExpressApi(request, '/v1/inventory/purchase-orders', {
      method: 'POST',
      headers: buildProxyHeaders(request, authToken),
    ...
    if (!upstream.ok) {
      return NextResponse.json(upstreamError(data, 'Failed to create purchase order'), {
        status: upstream.status,
      });
    }
