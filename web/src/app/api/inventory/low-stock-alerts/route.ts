...
export async function GET(request: NextRequest) {
  try {
    ...
    const upstream = await forwardToExpressApi(request, '/v1/inventory/low-stock-alerts', {
      method: 'GET',
    ...
    if (!upstream.ok) {
      return NextResponse.json(upstreamError(data, 'Failed to fetch low stock alerts'), {
