...
export async function GET(request: NextRequest) {
  try {
    ...
    const expressPath = queryString ? `/v1/menus/items?${queryString}` : '/v1/menus/items';
    ...
    if (!upstream.ok) {
      return NextResponse.json(upstreamError(data, 'Failed to fetch daily menu'), {
        status: upstream.status,
      });
    }

    return NextResponse.json(normalizeProxyResponse(data, 'Daily menu retrieved successfully'), {
      status: upstream.status,
    });
  } catch {
