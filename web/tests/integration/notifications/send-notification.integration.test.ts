...
test.describe(_'Notification Send API Integration', _() => {
  ...
  test.describe(_'Internationalization', _() => {
    test(_'should handle Unicode characters in messages', _async ({ request }) => {
      const _notificationRequest =  testData.createNotificationRequest({
        message: 'Hello 世界! 🌍 Test with émojis and spëcial chärs 🚀',
        subject: 'Unicode Test ユニコード'
      });
      ...
      const responseData: _NotificationResponse =  await response.json();
      expect(responseData.success).toBe(true);
      expect(responseData.data.supportsUnicode).toBe(true);
    });

    test(_'should handle right-to-left text', _async ({ request }) => {
      const _notificationRequest =  testData.createNotificationRequest({
        message: 'مرحبا بالعالم! Hello World! שלום עולם!',
        type: 'sms'
      });
      ...
      const responseData: _NotificationResponse =  await response.json();
      expect(responseData.success).toBe(true);
      expect(responseData.data.textDirection).toBeDefined();
    });
  });
});