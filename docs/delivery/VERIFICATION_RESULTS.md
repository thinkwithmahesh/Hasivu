...
**Run date (UTC):** 2026-05-18 (updated after Apple Silicon Docker build fix)  
...
| 4   | Frontend Docker/Webpack build    | **PASS** | `cd web && npm run build:docker` uses `next build --webpack`                         |
...
| 16  | npm audit                        | **PASS** | Root + web: `found 0 vulnerabilities`                                                |
| 17  | Docker frontend image build      | **PASS** | `docker compose -f docker-compose.dev.yml build frontend` completed on Apple Silicon |
...
cd web && npm run type-check && npm run build:docker
cd web && npm test -- --passWithNoTests
cd ..

npm audit --audit-level=moderate
cd web && npm audit --audit-level=moderate && cd ..
...
## Apple Silicon Docker Build

The frontend Docker build now runs:

```bash
npm run build:docker
# generate docs/assets, then: next build --webpack
```

This avoids the Next/Turbopack native binding failure on `linux/arm64`:

```text
Turbopack is not supported on this platform (linux/arm64)
```
...
# Result: Image hasivu-platform-frontend Built
```

During that Docker build, frontend `npm ci` reported:
