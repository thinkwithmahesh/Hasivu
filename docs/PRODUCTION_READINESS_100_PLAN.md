...
| T4  | **Playwright a11y** job against **preview/staging** URL (non-blocking → blocking).                       | Eng       | **In progress (blocked on remote setup):** workflow file exists locally and must be pushed; set `PLAYWRIGHT_BASE_URL` secret, then run once successfully. Setup guide: `docs/quality/PLAYWRIGHT_A11Y_CI_SETUP.md`.          |
