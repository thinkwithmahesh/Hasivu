...
export const handler = async (
  ...
  try {
    ...
    logger.error('Multi-school orchestrator request failed', {
      requestId,
...
async function getOrchestrationJob(
  ...
  try {
    ...
    if (!job.length) {
      return createErrorResponse('Orchestration job not found', 404, 'JOB_NOT_FOUND');
    }
  ...
  try {
    ...
    if (user.role !== 'super_admin' && user.districtId) {
      ...
      if (schoolCheck.length !== requestData.targetSchools.length) {
        return createErrorResponse('Access denied to some target schools', 403, 'ACCESS_DENIED');
      }
    }
...
async function scheduleOperation(
  ...
  try {
    if (!requestData.scheduledAt) {
      return createErrorResponse('Scheduled time is required', 400, 'VALIDATION_ERROR');
    }
...
async function updateOrchestrationJob(
  ...
  try {
    ...
    if (!result.length) {
      return createErrorResponse('Job not found or cannot be updated', 404, 'JOB_NOT_UPDATABLE');
    }
...
async function deleteOrchestrationJob(
  ...
  try {
    ...
    if (!result.length) {
      return createErrorResponse('Job not found or cannot be deleted', 404, 'JOB_NOT_DELETABLE');
    }
