...
export const handler = async (
  ...
  try {
    ...
    switch (httpMethod) {
      ...
      default:
        return createErrorResponse('Method not allowed', 405, 'TENANT_METHOD_NOT_ALLOWED');
    }
  } catch (error: unknown) {
    const duration = Date.now() - startTime;
    logger.error('tenantManagerHandler failed', {
      statusCode: 500,
      duration,
...
async function createTenant(tenantData: any): Promise<APIGatewayProxyResult> {
  try {
    ...
    if (!validation.isValid) {
      return createErrorResponse('Invalid tenant data', 400, 'TENANT_VALIDATION_FAILED');
    }
