...
export const handler = async (
  ...
  try {
    ...
    logger.error('Enterprise billing consolidation request failed', {
      requestId,
...
async function getUsageMetrics(
  ...
  try {
    ...
    if (!districtId && user.role !== 'super_admin') {
      return createErrorResponse('District ID required', 400, 'DISTRICT_REQUIRED');
    }
...
async function generateConsolidatedInvoice(
  ...
  try {
    ...
    const billingConfig = usageData.billingConfig;
    ...
    if (includeUsage) {
      // Usage-based charges
      const charges = usageData.calculatedCharges.charges;
...
async function calculateUsageCharges(
  ...
  try {
    ...
    if (!metrics || !config) {
      return createErrorResponse('Metrics and configuration are required', 400, 'VALIDATION_ERROR');
    }
...
async function sendInvoice(
  ...
  try {
    ...
    if (!invoiceId || !recipients) {
      return createErrorResponse('Invoice ID and recipients are required', 400, 'VALIDATION_ERROR');
    }
