...
export const handler = async (
  ...
  try {
    ...
    logger.error('Cross-school analytics request failed', {
      requestId,
...
async function executeCustomQuery(
  ...
  try {
    ...
    if (!query.metric || !query.timeRange) {
      return createErrorResponse('Metric and time range are required', 400, 'VALIDATION_ERROR');
    }
...
async function exportAnalyticsData(
  ...
  try {
    ...
    if (!['CSV', 'JSON', 'EXCEL'].includes(format)) {
      return createErrorResponse('Invalid export format', 400, 'INVALID_FORMAT');
    }
