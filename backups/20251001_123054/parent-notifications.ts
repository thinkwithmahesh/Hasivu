...
async function getMobileNotifications(
  ...
  if (notificationType) {
    whereClause._type =  notificationType;
  }

  if (dateFrom || dateTo) {
    whereClause._createdAt =  {};
    if (dateFrom) whereClause.createdAt._gte =  dateFrom;
    if (dateTo) whereClause.createdAt._lte =  dateTo;
  }
  ...
  // Format response
  const formattedNotifications: MobileNotificationResponse[] = notifications.map(_notification = > {
    let data 
    try {
...
async function handleSendNotification(
  ...
  logger.info('Mobile notifications sent', {
    ...
    successCount: results.filter(_r = > r.status 
