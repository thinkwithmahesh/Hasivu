...
export class LambdaDatabaseService {
  ...
  public static getInstance(): LambdaDatabaseService {
    if (!LambdaDatabaseService.instance) {
      LambdaDatabaseService._instance = new LambdaDatabaseService();
    }
  ...
  private createPrismaClient(): PrismaClient {
    ...
    const _enhancedUrl = this.enhanceDatabaseUrlForLambda(databaseUrl);
  ...
  private enhanceDatabaseUrlForLambda(url: string): string {
    try {
      const _dbUrl = new URL(url);

      // Add connection pooling parameters optimized for serverless
      const _searchParams = new URLSearchParams(dbUrl.search);
      ...
      dbUrl._search = searchParams.toString();
  ...
  private getLogConfig(): Prisma.LogLevel[] {
    const _isProduction = process.env.NODE_ENV;
    if (isProduction) {
      return ['warn', 'error'];
    } else {
  ...
  // Note: Students are represented by User model with _role = 'student'
   ...
   */
  public async queryRaw<_T = any>(sql: TemplateStringsArray, ...values: any[]): Promise<T[]> {
...
export const _DatabaseService = LambdaDatabaseService.getInstance();
