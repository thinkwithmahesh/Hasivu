...
const env = cleanEnv(process.env, {
  ...
  JWT_SECRET: str({
    default: 'test-jwt-secret-key-for-testing-purposes-only-must-be-changed-in-production',
    desc: 'JWT secret must be at least 64 characters for production security',
  }),
  JWT_REFRESH_SECRET: str({
    default: '',
    desc: 'Separate secret for refresh tokens (optional, will use JWT_SECRET + suffix)',
  }),
  ...
  SESSION_SECRET: str({
    default: 'test-session-secret-key-for-testing-only',
    desc: 'Session secret must be at least 32 characters',
  }),
  CORS_ORIGINS: str({ default: 'http://localhost:3000,http://localhost:3001' }),
  TRUSTED_PROXIES: str({ default: '' }),
  ENCRYPTION_KEY: str({
    default: 'test-encryption-key-for-testing-must-be-32-chars',
    desc: 'Data encryption key for sensitive information',
  }),
  ...
  AWS_ACCESS_KEY_ID: str({
    default: 'test-aws-access-key',
    desc: 'AWS access key ID for S3 and SES',
  }),
  ...
  RAZORPAY_KEY_ID: str({
    default: 'rzp_test_1234567890',
    desc: 'Razorpay key ID must be valid format',
  }),
  RAZORPAY_KEY_SECRET: str({
    default: 'test_secret_1234567890',
    desc: 'Razorpay key secret must be valid format',
  }),
  RAZORPAY_WEBHOOK_SECRET: str({
    default: 'test-webhook-secret-32-characters-long',
    desc: 'Webhook secret must be at least 32 characters',
  }),
  ...
  WHATSAPP_ACCESS_TOKEN: str({
    default:
      'test-whatsapp-access-token-1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890',
    desc: 'WhatsApp access token must be valid format',
  }),
  WHATSAPP_PHONE_NUMBER_ID: str({
    default: 'test-phone-number-id',
    desc: 'WhatsApp phone number ID must be valid',
  }),
  WHATSAPP_WEBHOOK_VERIFY_TOKEN: str({
    default: 'test-webhook-verify-token',
    desc: 'WhatsApp webhook verify token',
  }),
  WHATSAPP_BUSINESS_ACCOUNT_ID: str({
    default: 'test-business-account-id',
    desc: 'WhatsApp business account ID',
  }),
  // SendGrid
  SENDGRID_API_KEY: str({
    default: 'test-sendgrid-api-key',
    desc: 'SendGrid API key for email services',
  }),
  ...
  FIREBASE_DATABASE_URL: url({
    default: 'https://test-project.firebaseio.com',
...
export function validateConfig(): void {
  ...
  if (config.server.nodeEnv === 'production') {
    ...
    if (
      config.jwt.secret.toLowerCase().includes('secret') ||
