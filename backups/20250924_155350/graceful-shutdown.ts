 ...
 */
const connections: Set<ConnectionInfo> = new Set();
...
async function drainConnection(
  ...
  opts: Required<GracefulShutdownOptions>
): Promise<void> {
...
async function shutdownServices(opts: Required<GracefulShutdownOptions>): Promise<void> {
  ...
  try {
    ...
    for (const priority of opts.shutdownPriority) {
      ...
      opts.logger.info(
        `Shutting down priority ${priority} services (${services.length} services)...`
      );
...
async function shutdownService(
  ...
  try {
    ...
    if (config.dependencies && config.dependencies.length > 0) {
      const unmetDependencies = config.dependencies.filter(
        dep => !shutdownStatus.completedServices.includes(dep)
      );

      if (unmetDependencies.length > 0) {
        opts.logger.warn(
          `Service ${config.name} has unmet dependencies: ${unmetDependencies.join(', ')}`
        );
      }
    }
  ...
  return async function gracefulShutdown(signal: string): Promise<void> {
    ...
    try {
      ...
      const shutdownDuration = shutdownStatus.startTime
        ? Date.now() - shutdownStatus.startTime.getTime()
        : 0;
...
export class TimeoutManager {
   ...
   */
  public static setTimeout(id: string, callback: () => void, ms: number): NodeJS.Timeout {
